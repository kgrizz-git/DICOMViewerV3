...
### Dual P1 parallel slice — **2026-06-04** (user)

**Success criteria:** Two **parallel** workstreams on **disjoint modules** (menu coordination: Stream A **View → Layout**; Stream B **File → Export** only):

1. **LP-A — Three-pane asymmetric layouts** (`dev-docs/TO_DO.md` ~L113): four 3-pane modes (`1+2R`, `2L+1`, `2T+1`, `1+2B`); key **3** cycles 3-pane family; key **2** toggles **1×2** ↔ **2×1** (from 3-pane → last 2-pane); menus + slot map + config persistence; unit tests in **`tests/test_multi_window_layout.py`**. Plan: **`dev-docs/plans/supporting/THREE_PANE_ASYMMETRIC_LAYOUT_PLAN.md`** (T0 locked 2026-06-04).

2. **LP-B — Deep Anonymizer export** (`dev-docs/TO_DO.md` ~L142): profile + **`DeepDICOMAnonymizer`**; **File → Export → Export with Deep Anonymization…**; options dialog (all strip options **on** by default); wire **`export_manager.py`** (not raster **`export_rendering.py`**); **`PatientIdentityRemoved`** + **`DeidentificationMethod`** tags; unit tests **`tests/test_deep_anonymizer.py`**. Plan: **`dev-docs/plans/supporting/DEEP_ANONYMIZER_EXPORT_PLAN.md`** (path correction 2026-06-04).

**Verification:** targeted **`pytest`** per stream; batch **`tester`** at slice end (**`medium`**); manual smoke if UI-heavy (**`dev-docs/orchestration/AGENT_SMOKE.md`**). **CHANGELOG** + **`src/version.py`** patch if user-facing ships. **Backups:** `backups/<file>.bak-<stream>-YYYYMMDD-HHMMSS.bak` before each `.py` edit. **Venv:** `.venv\Scripts\python.exe`. **Do not commit/push.**
...
- **`LP-A` / `LP-B` (2026-06-04):** **`medium`** — UI layout + PHI export stripping; batch **`tester`** at fan-in.
...
| Field | Value |
|-------|-------|
| Orchestrator cycles (this run) | 37 |
| Max orchestrator cycles | 40 |
| Specialist completions (this run) | 29 |
...
| Stream | Scope | Status |
|--------|--------|--------|
| **LP-A** | **Three-pane asymmetric layouts** | **complete** | — |
| **LP-B** | **Deep Anonymizer export** | **complete** | — |
...
|----|-------|------|--------------|--------|
| **LP-A** | **coder** | **P1** Three-pane asymmetric layouts (T1–T7, T9) | `dev-docs/plans/supporting/THREE_PANE_ASYMMETRIC_LAYOUT_PLAN.md` | **done** (tester green; T8/T10 manual open) |
| **LP-B** | **coder** | **P1** Deep Anonymizer export (Phases 1–3) | `dev-docs/plans/supporting/DEEP_ANONYMIZER_EXPORT_PLAN.md` | **done** (tester green; Phase 4 manual open) |
| **LP-T** | **tester** | Dual P1 fan-in batch pytest | `logs/test-ledger.md` | **done** |
| **LP-R** | **reviewer** | Dual P1 spec alignment | plans + touch-set | **done** (**yes_with_followups**) |
...
1. **Dual P1 slice `complete`** — resume backlog (**`T4`**, **`T14`**, **`SD8`**) or user picks next slice; optional manual smoke per reviewer (no **`ux`** dispatch required).
2. **Optional coder follow-ups:** context-menu layout checkmarks, slot-map hidden-pane dimming, two free-text tags in deep profile.
3. **Do not commit/push** unless user asks.

## Session checkpoint

- **2026-06-04 — Dual P1 slice `complete`:** LP-A + LP-B + tester + reviewer **yes_with_followups**. **856** pytest green. **v0.3.2**. Cycle **37**; completions **29**. **Chain** **`autonomous`** → **`NEXT_TASK_TOOL: none`**.
...
### 2026-06-04 — reviewer (Dual P1 LP-A + LP-B)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: medium
- Stream: LP-A, LP-B
- Artifacts: dev-docs/plans/supporting/THREE_PANE_ASYMMETRIC_LAYOUT_PLAN.md (completion notes); dev-docs/plans/supporting/DEEP_ANONYMIZER_EXPORT_PLAN.md (Phase 4 regression [x])
- Plan deltas: LP-A T1–T7/T9 verified [x]; G1 closed; T8/T10/G2 manual open. LP-B Phases 1–3 verified [x]; Phase 4 manual QA open; Phase 4 export-regression [x]; Phase 3 navigator context menu open
- Pytest: reviewer re-ran targeted 17/17 pass (layout 10 + deep_anonymizer 7); aligns with tester 856 full suite
- Lint: read_lints clean on touched LP-A/LP-B core files; basedpyright unavailable (venv module `./dist/pyright` missing — non-blocking)
- Findings (priority): (1) LP-A context-menu layout checkmarks not synced on open — View menu OK (coder-known). (2) LP-A slot map: 3 active overlay OK; hidden slot not dimmed vs plan “ghosted”. (3) LP-B FREE_TEXT_TAGS omits ContentDescription (0008,0088) and StudyComments (0032,4000) from plan minimum list — minor PHI gap. (4) Menu split correct: View→Layout (LP-A), File→Export deep entry (LP-B). (5) Deep export: PS3.15 tags, batch UID/date, export_manager path, dialog defaults all on, burned-in warning — verified in code + tests
- Risks: none blocking merge; manual smoke still recommended for layout cycle + deep export reload
- Recommended next: optional Task(ux) manual smoke (AGENT_SMOKE bullets from tester); then slice complete. Follow-up coder items optional: context-menu checkmark sync, slot-map dim hidden cell, add missing free-text tags
- Git: dirty; branch: workspace; worktree: none
- Git proposal: none
- PR: none
- Cloud: none
...
### 2026-06-04 — tester (Dual P1 LP-A + LP-B fan-in)
...
- Pytest: .venv\Scripts\python.exe -m pytest tests/ -v — 856 passed, 21 warnings, ~82 s; targeted tests/test_multi_window_layout.py + tests/test_deep_anonymizer.py — 17 passed, ~2.5 s
- Plan alignment: LP-A promised tests/test_multi_window_layout.py (10 tests) — present, green; LP-B promised tests/test_deep_anonymizer.py (7 tests) — present, green; plan T8/T10 manual QA still open
- Risks: none (automated gate); context menu checkmark sync + 3-pane swap deferred per coder notes — manual smoke recommended
- Recommended next: Task(reviewer) on LP-A + LP-B touch-set; optional Task(ux) manual smoke (keys 2/3/4, File→Export deep dialog)
- Suggested manual smoke: (1) keys 2/3/4 — 1×2↔2×1 toggle, 3-pane cycle (1+2R→2L+1→2T+1→1+2B), 2×2; (2) File→Export→Export with Deep Anonymization… — all strip options on by default; export CT/MR sample; reload and confirm institution/patient stripped, PatientIdentityRemoved + DeidentificationMethod set
- Git: dirty; branch: workspace; worktree: none
- Git proposal: none
- PR: none
- Cloud: none
- Merge recommendation: n/a (tester)
```

### 2026-06-04 — coder (LP-B Deep Anonymizer export)
...
- Execution mode: full | Risk tier: medium
- Stream: LP-B
- Artifacts: src/utils/deep_anonymizer_profile.py, src/utils/deep_anonymizer.py, src/utils/dicom_vr_helpers.py, src/utils/dicom_anonymizer.py, src/gui/export_manager.py, src/gui/dialogs/deep_anonymizer_export_dialog.py, src/gui/main_window_menu_builder.py, src/gui/main_window.py, src/gui/export_app_facade.py, src/gui/actions/dialog_actions.py, src/gui/dialog_coordinator.py, src/gui/app_signal_wiring.py, src/main.py, tests/test_deep_anonymizer.py, dev-docs/plans/supporting/DEEP_ANONYMIZER_EXPORT_PLAN.md, dev-docs/TO_DO.md, CHANGELOG.md; backups/*bak-lp-b-20260604-194315*
- Pytest: .venv\Scripts\python.exe -m pytest tests/test_deep_anonymizer.py -v — 7 passed
- Plan deltas: Phases 1–3 [x]; Phase 4 manual QA open; TO_DO L142 [x]
- Risks: navigator context menu export deferred; burned-in PHI warning only
- Recommended next: Task(tester) batch with LP-A; reviewer; ux smoke (File→Export deep dialog)
- Git: dirty; branch: workspace; worktree: none
- Git proposal: none
- PR: none
- Cloud: none
- Merge recommendation: pending tester + reviewer
```

### 2026-06-04 — coder (LP-A three-pane layouts)

```text
HANDOFF → orchestrator:
- Status: done
- Artifacts: src/gui/multi_window_layout.py, src/utils/config/layout_config.py, keyboard_event_handler.py, main_window_menu_builder.py, image_viewer_context_menu.py, window_slot_map_widget.py, main_window.py, app_handler_bootstrap.py; tests/test_multi_window_layout.py; CHANGELOG.md; src/version.py 0.3.2; dev-docs/TO_DO.md L113 [x]; plan T1–T7/T9 [x]
- Plan deltas: THREE_PANE Phases 1–2 complete; T8/T10 deferred tester
- Risks: v1 no swap in 3-pane (2×2 swap unchanged); context menu checkmarks not synced to current mode on open
- Recommended next: Task(tester) — pytest batch + manual T8/T10 smoke (keys 2/3/4, hidden pane assignment)
- Stream: LP-A
- Git: dirty; branch: workspace; worktree: none
- Git proposal: none
- PR: none
...
### 2026-06-04 — orchestrator (Dual P1 resume — cycle 34)

```text
HANDOFF → orchestrator:
- Status: done
- Artifacts: plans/orchestration-state.md (progress snapshot)
- Plan deltas: none verified — checklists unchanged pending coder completion
- Risks: coders partial WIP; no tests; fan-in blocked until both HANDOFF done
- Recommended next: wait for LP-A/LP-B coder HANDOFF; then Task(tester) medium batch
- Stream: none
- Git: dirty (WIP); branch: workspace; worktree: none
- Git proposal: none
- PR: none
- Cloud: none
- Merge recommendation: n/a
...
### 2026-06-04 — orchestrator (Dual P1 parallel slice kickoff)

```text
HANDOFF → orchestrator:
- Status: done
- Artifacts: dev-docs/plans/supporting/THREE_PANE_ASYMMETRIC_LAYOUT_PLAN.md (T0 locked, +layout_config.py); dev-docs/plans/supporting/DEEP_ANONYMIZER_EXPORT_PLAN.md (export_manager path fix); plans/orchestration-state.md
- Plan deltas: THREE_PANE T0 [x]; DEEP_ANON files table corrected
- Risks: main_window_menu_builder.py shared file — section split enforced (View→Layout vs File→Export)
- Recommended next: parallel Task(coder) LP-A + LP-B — in flight; fan-in Task(tester) medium after both done
- Stream: none
- Git: dirty expected; branch: user workspace; worktree: none
- Git proposal: none
- PR: none
- Cloud: none
- Merge recommendation: n/a
...
### 2026-06-04 — coder (TO_DO hygiene + W/L UI layout + status bar W/L)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: fast | Risk tier: low
- Artifacts: dev-docs/TO_DO.md (Changes + session notes), src/gui/window_level_controls.py, src/gui/main_window_layout_helper.py, src/gui/main_window.py, src/core/wl_preset_catalog.py, src/core/view_state_handlers.py, src/core/window_level_preset_handler.py, src/core/slice_window_level_resolver.py, src/gui/view_state_manager.py, src/main.py, tests/test_window_level_controls.py, tests/test_wl_preset_catalog.py, tests/test_window_level_preset_handler.py, CHANGELOG.md; backups/*wl-ui-20260604-174004*
- TO_DO: Changes line updated 2026-06-04 (status bar W/L, Presets top row, cut/paste + ROI handle fixes noted); prior session items already [x] (L50 cancel load, L55 W/L menu C/W, L106 Cut, L123 large-file cancel)
- Pytest: .\.venv\Scripts\python.exe -m pytest tests/test_window_level_controls.py tests/test_wl_preset_catalog.py tests/test_window_level_preset_handler.py -v — 41 passed
- Recommended next: reviewer (optional), tester batch if slice-end gate, ux smoke right-pane Presets row + status bar W/L on preset/slider/rescale
- Git: dirty; no commit/push
- Git proposal: none
- PR: none
- Cloud: none
```

### 2026-06-04 — coder (cancel-load toast + W/L compact label spacing)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: fast | Risk tier: low
- Artifacts: src/gui/file_series_loading_coordinator.py, src/core/wl_preset_catalog.py, tests/test_wl_preset_catalog.py, user-docs/CONFIGURATION.md, CHANGELOG.md; backups/*ux-toast-wl-20260604-173542*
- Changes: cancel-load index-skip toast → "Folder loading canceled — study not added to index"; W/L menu compact suffix `(W 400/C 40)` spacing; docs/changelog strings updated
- Pytest: .\.venv\Scripts\python.exe -m pytest tests/test_wl_preset_catalog.py tests/test_canceled_load_behavior.py tests/test_wl_preset_menu_access.py -v — 31 passed
- Recommended next: reviewer (optional) or user manual smoke cancel folder load + W/L menu labels
- Git: dirty; no commit/push
- Git proposal: none
- PR: none
- Cloud: none
...
### 2026-06-04 — coder (W/L preset menu access — View / right pane / Quick W/L)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: low
- Artifacts: src/gui/wl_preset_menu.py, src/gui/window_level_controls.py, src/gui/dialogs/quick_window_level_dialog.py, src/gui/actions/dialog_actions.py, src/gui/main_window_menu_builder.py, src/gui/main_window_toolbar_builder.py, src/main.py, tests/test_wl_preset_menu_access.py, CHANGELOG.md, user-docs/USER_GUIDE.md; backups/*wl-preset-access-20260604-173047*
- Changes: wire_dynamic_wl_preset_menu + create_wl_presets_menu_button; View → Window/Level Presets; right-pane Presets…; Quick W/L Presets… (applies + closes); toolbar refactored to shared wire
- Pytest: .\.venv\Scripts\python.exe -m pytest tests/test_wl_preset_catalog.py tests/test_window_level_controls.py tests/test_wl_preset_menu_access.py -v — 38 passed
- Recommended next: reviewer (optional); ux smoke View menu + right pane + Quick W/L presets on CT sample
- Git: dirty; no commit/push
- Git proposal: none
- PR: none
- Cloud: none
...
### 2026-06-04 — coder (cancel-index toast + W/L label compact)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: low
- Artifacts: src/gui/file_series_loading_coordinator.py, src/main.py, src/core/wl_preset_catalog.py, tests/test_wl_preset_catalog.py, user-docs/CONFIGURATION.md, CHANGELOG.md; backups/*20260604-172900*
- Changes: centered warning toast on partial cancel (index skip); W/L menu labels `(W400/C40)`; docs/changelog notes
- Pytest: .\.venv\Scripts\python.exe -m pytest tests/test_wl_preset_catalog.py tests/test_canceled_load_behavior.py -v — 27 passed
- Recommended next: reviewer (optional); ux manual smoke cancel folder + W/L menu labels
- Git: dirty; no commit/push
- Git proposal: none
- PR: none
- Cloud: none
...
### 2026-06-04 — coder (Track B: W/L preset menu C/W values)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: medium
- Artifacts: src/core/wl_preset_catalog.py, src/gui/wl_preset_menu.py, tests/test_wl_preset_catalog.py, dev-docs/TO_DO.md, CHANGELOG.md, dev-docs/plans/supporting/WL_PRESET_MENU_VALUES_PLAN.md; backups/wl_preset_catalog.py.bak-wl-menu-values-*, wl_preset_menu.py.bak-wl-menu-values-*
- Plan deltas: WL_PRESET_MENU_VALUES T1–T6 [x]; T7 manual QA open
- Pytest: .\.venv\Scripts\python.exe -m pytest tests/test_wl_preset_catalog.py -v — 23 passed
- Recommended next: reviewer (spec alignment); tester T7 manual smoke (toggle rescaled, reopen menu)
- Stream: Track B WL menu values (no loading_pipeline overlap)
- Git: dirty; no commit/push
- Git proposal: none
- PR: none
- Cloud: none
...
### 2026-06-04 — tester (full `pytest tests/` — user request)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: medium
- Artifacts: logs/test-ledger.md (row 2026-06-04)
- Plan deltas: none
- Risks: venv was incomplete before run (`annotated_types`, `argue` missing); repaired via pip only — recommend `pip install -r requirements.txt` on fresh clones
- Pytest: `.\.venv\Scripts\python.exe -m pytest tests/ -v` — **819 passed**, **0 failed**, **0 skipped**, **103.21 s**; **22** warnings (pydicom VR UI in study-cache; requests charset — existing)
- Collection note: first attempt **ERROR** `tests/test_pylinac_extent_subclasses.py` (`ModuleNotFoundError: annotated_types`); unblocked after pip repair, full suite green
- Recommended next: user manual smoke per AGENT_SMOKE.md (out of scope this run); if verifying 2026-06-04 coder slices (large-file warning, annotation cut), dispatch **reviewer** on touched paths + targeted manual bullets from coder HANDOFFs
- Stream: none
...
### 2026-06-04 — coder (Track A: large-file cancel before load)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: medium
- Changed: src/gui/file_operations_handler.py; src/gui/dialogs/file_dialog.py; tests/test_large_file_warning.py; dev-docs/TO_DO.md [x]; dev-docs/plans/supporting/NAVIGATOR_AND_FILE_LOADING_FEEDBACK_PLAN.md §3; CHANGELOG.md; src/version.py 0.3.1; backups/*largefile-20260604
- Tests: .\.venv\Scripts\python.exe -m pytest tests/test_large_file_warning.py -v — 7 passed
- Lint: read_lints clean on touched modules
- Recommended next: reviewer (plan §3 alignment), tester optional (manual: open >25 MB file, Cancel vs Continue)
- Git proposal: none
- Cloud: none
```

### 2026-06-04 — coder (Track B: Annotation Cut Ctrl+X)

```text
HANDOFF → orchestrator:
- Status: done
- Execution mode: full | Risk tier: low
- Changed: src/gui/annotation_paste_handler.py (cut_annotations, _delete_selected_annotations); src/gui/main_window.py (cut_annotation_requested signal); src/gui/main_window_menu_builder.py (Edit → Cut); src/gui/app_signal_wiring.py; src/gui/dialogs/keyboard_shortcuts_dialog.py; tests/test_annotation_cut.py; dev-docs/TO_DO.md [x]; dev-docs/plans/supporting/ANNOTATION_CUT_CLIPBOARD_PLAN.md; CHANGELOG.md; src/version.py 0.3.1
- Tests: .\.venv\Scripts\python.exe -m pytest tests/test_annotation_cut.py -v — 2 passed
- Lint: read_lints clean on touched handler + test
- Recommended next: reviewer (spec alignment), then tester (T7 manual smoke: cut/paste/undo)
- Git proposal: none (single-stream)
- Cloud: none
```
...
### 2026-06-04 — coder (**Track A — canceled folder load**)

- **Scope:** `src/core/loading_pipeline.py`, `src/core/study_index/index_service.py`, `src/main.py`, `src/gui/dialogs/study_index_search_dialog.py`, `tests/test_canceled_load_behavior.py`, `dev-docs/plans/supporting/CANCELED_LOAD_BEHAVIOR_PLAN.md`, `dev-docs/TO_DO.md`, `user-docs/CONFIGURATION.md`, `CHANGELOG.md` [Unreleased]; backups `backups/*.bak-cancel-load-20260604-172201.bak`.
- **Verify:** `python -m pytest tests/test_canceled_load_behavior.py -v` → **4 passed**; `python scripts/check_user_docs_links.py` → OK.

```text
HANDOFF → orchestrator:
- Status: done
- Artifacts: loading_pipeline (was_cancelled + format_cancelled_partial_status); index_service schedule_index_after_load skip; study_index_search_dialog folder rescan; tests/test_canceled_load_behavior.py (4); plan findings; TO_DO L50 [x]; CONFIGURATION.md; CHANGELOG Fixed
- Q1 partial auto-index on cancel: was YES, now NO (skip index; viewer partial kept)
- Q2 re-open from index: was indexed path list only; now full load_directory when study_root_path is dir; recent menu already full rescan
- Plan deltas: CANCELED_LOAD_BEHAVIOR_PLAN Phases 1–5 [x] except optional manual test
- Risks: legacy partial index rows before fix still open via file list if study_root missing on disk — edge only
- Recommended next: reviewer (quick) or tester batch if slice requires full pytest
- Stream: A
- Git: dirty; branch: unknown; worktree: none
- Git proposal: none
- PR: none
...
### 2026-07-11 — coder (Sequence Tag Viewer plan Phase 2: shared tree helpers + tag viewer dialog)

- **Scope:** `src/gui/metadata_table_model.py` (`metadata_row_depth`/`metadata_row_parent_key`/`metadata_row_kind`, `group_metadata_tags_sorted` now depth-0-only, `filter_metadata_tags_by_search` now retains ancestor chains, new `get_metadata_tag_children`); `src/gui/dialogs/tag_viewer_dialog.py` (Show Sequences checkbox default on, Mode B tree building via `parent_key`, Expand All/Collapse All buttons, `_is_editable_item` allow-list replacing `parent() is None` edit gating); `tests/test_metadata_table_model.py` (+7 tests); `tests/test_tag_viewer_dialog.py` (+5 tests incl. nested-edit regression + perf gate); `dev-docs/plans/supporting/SEQUENCE_TAG_VIEWER_AND_GROUP_COLLAPSE_PLAN.md` (Phase 2 checkboxes `[x]`, PERF FINDING note, open-question 3 resolved).
- **Verify:** `ruff check` on touched files → clean; `python -m pytest tests/ -q` → **1856 passed, 18 skipped, 1 xfailed** (the perf-gate test), 0 failed.

```text
HANDOFF → orchestrator:
- Status: blocked
- Execution mode: unknown | Risk tier: unknown (not set in this file's orchestrator-owned sections at time of this pass)
- Artifacts: src/gui/metadata_table_model.py; src/gui/dialogs/tag_viewer_dialog.py; tests/test_metadata_table_model.py; tests/test_tag_viewer_dialog.py; dev-docs/plans/supporting/SEQUENCE_TAG_VIEWER_AND_GROUP_COLLAPSE_PLAN.md
- Plan deltas: Phase 2 checklist items [x]; Phase 2 verification bullets [x]; open question 3 (row cap / eager vs lazy) resolved — sluggish, confirmed
- Blocker (the reason for "blocked"): PERF GATE FAILED per plan's own instruction. Timed QTreeWidgetItem construction for the 24,001-row synthetic 2000-frame Mode B dataset (same fixture as the parser's `test_get_all_tags_mode_b_perf_gate_enhanced_multiframe`): **~19.7s**, ~20x over the plan's 1s budget. Per the plan and my task instructions, I did NOT implement the prescribed fix (lazy child population on first expand) because it's flagged as a bigger design change needing approval first. Pinned as `tests/test_tag_viewer_dialog.py::test_widget_population_perf_gate_enhanced_multiframe`, marked `xfail(strict=True)` so the full suite stays green (1856 passed / 0 failed) while the finding stays visible.
- Risks: Phase 2 dialog is fully implemented and correct (nested tree, filter ancestor-retention, edit gating, nested-edit regression test all pass) but eager tree building will visibly hang the tag viewer dialog on very large enhanced/multi-frame studies (thousands of frames) until lazy population is designed and approved. Phase 3 (metadata_panel.py) reuses the same shared helpers and will inherit the identical Qt-construction cost concern — recommend deciding the lazy-population approach before or during Phase 3, not after.
- Recommended next: orchestrator decision on lazy child population design (approve scope before Phase 3), then reviewer for Phase 2 diff, then continue to Phase 3 per plan (NOT started, out of scope this pass)
- Stream: none
- Git: dirty; branch: sequence-tag-viewer; worktree: none
- Git proposal: none
- PR: none
- Cloud: none
- Merge recommendation: yes_with_followups
...
### 2026-07-11 — coder (Sequence Tag Viewer plan Phase 4: opt-in sequences in tag export — final phase)

- **Scope:** `src/gui/dialogs/tag_export_dialog.py` (Include sequences checkbox, default off; `_refresh_tag_tree`/`_render_tag_tree`/`_ensure_sequences_union` replace the old `_apply_merged_tags_to_tree`, since Mode A/Mode B use different key schemes and can't share a filter-in-place cache like the private-tags toggle does); `src/core/tag_export_union.py` (`include_sequences` param, drops `depth != 0` rows when on — nested item/leaf rows never reach the picker); `src/gui/dialogs/tag_export_union_worker.py` (same flag + depth filter); `src/core/tag_export_writer.py` (`include_sequences` threaded into `write_excel_file`/`write_csv_files`/`write_txt_files`/`_write_tag_export_sheet_rows`); `src/core/tag_export_controller.py` (new `include_sequences` field, not explicitly named in the plan bullet but required — the controller sits between dialog and writer); `src/core/tag_export_catalog.py` (removed `_EXPORTABLE_SEQUENCE_TAGS`, `_resolve_catalog`/`supplement_export_tags_dict` now gate SQ rows on `include_sequences` param, module docstring updated); `src/core/dicom_parser.py` (one-line: pass `include_sequences` through to `supplement_export_tags_dict`). Tests: `tests/test_tag_export_sequences_flag.py` (new, golden byte-identical regression + on-mode single-cell summary), `tests/test_tag_export_dialog_sequences_checkbox.py` (new, Qt checkbox/toggle), `tests/test_tag_export_catalog.py` (updated for flag-based mechanism, deid tag still catalog-reachable via `include_sequences=True`), `tests/test_tag_export_controller.py` (updated fake signatures + 1 new test). Golden fixtures: `tests/fixtures/tag_export_golden/off_default.{csv,txt}`. Plan checkboxes ticked (Phase 4 + its Verification bullet + "Full pytest from .venv").
- **Byte-identical proof (the critical guarantee):** `git stash`'d all working changes back to clean HEAD (`17d1d32`), ran the writer against a fixed dataset (scalar tag + `DeidentificationMethodCodeSequence` SQ tag + a flattened nested `CodeMeaning` leaf + a missing tag) to produce true pre-Phase-4 CSV/TXT/XLSX, `git stash pop`'d back, then re-ran the same inputs through the finished code with `include_sequences` omitted (true default). CSV and TXT matched **byte-for-byte**; XLSX cell values matched (raw bytes differ only in openpyxl's `docProps/core.xml` timestamp, confirmed via zip inspection — not a data difference). Committed the CSV/TXT as goldens; `tests/test_tag_export_sequences_flag.py` re-asserts this on every run.
- **Verify:** `ruff check` on all touched/new files → clean (8 pre-existing RUF015/B904 findings elsewhere in `tag_export_writer.py`/`tag_export_dialog.py`/`test_tag_export_catalog.py` confirmed present on HEAD before this pass via a second `git stash` check — not introduced, not fixed, out of scope). `python -m pytest tests/ -q` → **1877 passed, 18 skipped**, 0 failed, 0 xfail (baseline 1866/18/0; +11 net new tests, no regressions).
- **Plan finding (turned out to be wrong/incomplete):** the plan's "thread the flag through tag_export_union.py → tag_export_union_worker.py → tag_export_writer.py" undercounts the call chain — `tag_export_controller.py` sits between the dialog and the writer (dialog never calls the writer directly), so it also needed the field, or the flag would never reach the writer at all. Also added the flag to the dialog's variation-analysis preview's two local `get_all_tags` calls (cosmetic-only: without it, a selected SQ tag's preview row shows its raw key instead of its friendly name — doesn't affect exported data since the writer independently re-resolves values). Not threaded: `tag_export_analysis_service.analyze_tag_variations` — deliberately left on Mode A always, so any selected SQ tag is always bucketed "constant" (exported once per series from the first instance) rather than tracked for per-instance variation; this is consistent with "no row explosion" scope and avoids touching a function signature with test fakes elsewhere, but is worth flagging if a user ever wants per-instance-varying SQ summaries.

```text
HANDOFF → orchestrator:
- Status: done
- Artifacts: src/gui/dialogs/tag_export_dialog.py; src/core/tag_export_union.py; src/gui/dialogs/tag_export_union_worker.py; src/core/tag_export_writer.py; src/core/tag_export_controller.py; src/core/tag_export_catalog.py; src/core/dicom_parser.py; tests/test_tag_export_sequences_flag.py; tests/test_tag_export_dialog_sequences_checkbox.py; tests/test_tag_export_catalog.py; tests/test_tag_export_controller.py; tests/fixtures/tag_export_golden/off_default.{csv,txt}; dev-docs/plans/supporting/SEQUENCE_TAG_VIEWER_AND_GROUP_COLLAPSE_PLAN.md (Phase 4 checkboxes [x], Phase 4 + "Full pytest" verification [x])
- Plan deltas: Phase 4 all [x]; Verification "Phase 4" bullet [x]; "Whole slice" pytest bullet [x]. "Whole slice" manual smoke + CHANGELOG/version bump left unticked — not run/done this pass (need actual app launch + release-hygiene step, likely orchestrator/tester territory per Phase 2/3 precedent).
- Risks: none blocking. Minor: analyze_tag_variations always treats a selected SQ tag as constant (see plan finding above) — cosmetic/scope-consistent, not a correctness bug for the export file itself.
- Recommended next: reviewer (Phase 4 diff + full-plan alignment check now that all 4 phases are done), then tester for manual smoke (dev-docs/orchestration/AGENT_SMOKE.md) before CHANGELOG/version bump and merge
- Stream: none
- Git: dirty; branch: sequence-tag-viewer; worktree: none
- Git proposal: none
- PR: none
