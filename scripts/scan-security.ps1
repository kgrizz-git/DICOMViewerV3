...
# Build the Python command as an argument array (avoids Invoke-Expression)
$pythonArgs = @("scripts\run_security_scan.py")

if ($All -or (-not $Semgrep -and -not $Secrets -and -not $DebugFlags -and -not $Quick)) {
    $pythonArgs += "--all"
}
elseif ($Quick) {
    $pythonArgs += "--semgrep", "--debug-flags"
}
else {
    if ($Semgrep) { $pythonArgs += "--semgrep" }
    if ($Secrets) { $pythonArgs += "--secrets" }
    if ($DebugFlags) { $pythonArgs += "--debug-flags" }
    if ($Deps) { $pythonArgs += "--deps" }
}

if ($Verbose) { $pythonArgs += "--verbose" }
if ($Report) { $pythonArgs += "--report" }

Write-Header "Security Scanner"
Write-Host "Running: $pythonExe $($pythonArgs -join ' ')`n" -ForegroundColor DarkGray
