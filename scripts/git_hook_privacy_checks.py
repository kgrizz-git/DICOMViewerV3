...
Runs on staged Python sources under `src/` and staged docs/config-like text
files. Uses the staged index (`git show :path`) so checks match what will be
committed.
...
Rules (see dev-docs/QUICK_REFERENCE_SECURITY.md):
  ...
  - On added lines in docs/config-like files and scripts: machine-specific
    Windows or Unix home-directory paths unless explicitly allowed inline.
...
checks (sys.path includes repo/src for hook runs from repo root). Git output is
read as UTF-8 so Windows pre-commit hooks do not mis-decode staged sources.
"""
...
# Git emits repository paths and file blobs as UTF-8. On Windows, subprocess
# ``text=True`` defaults to the ANSI code page (often cp1252), which raises
# UnicodeDecodeError on UTF-8-only bytes (e.g. curly quotes in docstrings) and
# makes the hook report a false "git show :path failed".
...
def staged_doc_like_paths(root: Path) -> list[str]:
    ...
    for line in proc.stdout.splitlines():
        ...
        if rel.endswith(".py"):
            continue
        if rel.lower().endswith(_DOC_PATH_SUFFIXES):
            out.append(rel)
    return sorted(set(out))


def staged_absolute_path_check_paths(root: Path) -> list[str]:
    """Return staged docs/config and executable scripts that may embed local paths.

    Test fixtures intentionally use synthetic absolute paths to validate path
    sanitization, so they are excluded from this developer-machine path guard.
    """
    proc = subprocess.run(
        ["git", "-C", str(root), "diff", "--cached", "--name-only", "--diff-filter=ACM"],
        capture_output=True,
...
_PATH_HINT_RE = re.compile(
    r"(?:[A-Za-z]:\\\\|/Users/|/home/|\\\\Users\\\\|Documents\\\\|Downloads\\\\|Desktop\\\\)",  # privacy-hook: allow
    re.IGNORECASE,
)

_DOC_ABSOLUTE_PATH_RE = re.compile(
    r"(?:[A-Za-z]:\\(?:Users|Documents and Settings)\\|/Users/|/home/|\\\\[A-Za-z0-9][A-Za-z0-9._-]*\\)",  # privacy-hook: allow
    re.IGNORECASE,
)
...
def check_absolute_path_in_doc_line(relpath: str, line: str, lineno: int) -> list[Violation]:
    if _line_allowed(line):
        return []
    if "file://" in line.lower():
        return []
    if not _DOC_ABSOLUTE_PATH_RE.search(line):
        return []
    return [
        Violation(
            ...
            relpath,
            lineno,
            "Avoid machine-specific absolute paths in docs/config/scripts; prefer repo-relative paths unless an inline allow marker is justified.",
        )
    ]
...
def _is_logger_call(node: ast.Call) -> bool:
    ...
    if isinstance(val, ast.Name) and val.id == "logger":
        return True
    return bool(isinstance(val, ast.Attribute) and val.attr == "logger")
...
def _logger_call_violation(
    node: ast.Call, relpath: str, added_lines: set[int]
) -> Violation | None:
    """Return a Violation if this logger call needs sanitizing and lacks it, else None."""
    ...
    if len(node.args) > 1 and any(_expr_has_sanitize_call(a) for a in node.args):
        return None
    return Violation(
...
def check_logger_sanitize_ast(
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue
        violation = _logger_call_violation(node, relpath, added_lines)
...
def check_staged_doc_file(root: Path, relpath: str) -> list[Violation]:
    ...
    for lineno in sorted(added):
        if lineno < 1 or lineno > len(lines):
            continue
        violations.extend(check_absolute_path_in_doc_line(relpath, lines[lineno - 1], lineno))
    return violations
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Privacy checks for staged Python and docs/config files")
    ...
    path_check_paths = staged_absolute_path_check_paths(root)
    all_v: list[Violation] = []
    for rel in paths:
        all_v.extend(check_staged_file(root, rel))
    for rel in path_check_paths:
        all_v.extend(check_staged_doc_file(root, rel))
