...
# Windows/Linux: keep UPX enabled when the tool is installed (smaller on-disk layout).
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        'numpy',
        'PIL',
        # Tk/PIL.ImageTk not used (Qt UI + matplotlib qtagg only); PIL.Image stays for I/O;
        # PIL.ImageTk / PIL._tkinter_finder are in excludes (see pyinstaller_exclude_lists).
        'PIL.Image',
        # Plotting
        'matplotlib',
        # Histogram uses Qt6 canvas (see src/tools/histogram_widget.py); align with runtime, not qt5agg.
    ...
    excludes=(
        ...
        [
            ...
            'skimage.tests',
            'openpyxl.tests',
            # pylinac / imaging stack (direct or transitive); tests never needed frozen
            'pylinac.tests',
            'imageio.tests',
            'pandas.tests',
        ]
...
exe = EXE(
    ...
    strip=False,
    upx=USE_UPX,  # False on macOS for signing/notarization; True elsewhere if UPX is installed
