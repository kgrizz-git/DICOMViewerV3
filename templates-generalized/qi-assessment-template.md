...
#### 2.7: Python Type Checking (Pyright/Basedpyright)

**For Python repositories only**:

- [ ] Run pyright or basedpyright on all Python files (except any in a "backups" folder and any imported packages)
- [ ] Collect type checking output into timestamped file in `dev-docs/code-assessments/`
- [ ] Analyze type checking results for critical issues
- [ ] Document type-related errors and warnings

**Commands to run**:

**For Linux/macOS (bash/zsh)**:
```bash
# Create dev-docs/code-assessments directory if it doesn't exist
mkdir -p dev-docs/code-assessments

# Run pyright and save output to timestamped file
pyright . > "dev-docs/code-assessments/pyright-output-$(date +%Y-%m-%d-%H%M%S).txt" 2>&1

# OR run basedpyright if preferred
basedpyright . > "dev-docs/code-assessments/basedpyright-output-$(date +%Y-%m-%d-%H%M%S).txt" 2>&1
```

**For Windows (PowerShell)**:
```powershell
# Create dev-docs/code-assessments directory if it doesn't exist
New-Item -ItemType Directory -Force -Path "dev-docs/code-assessments"

# Run pyright and save output to timestamped file
$timestamp = Get-Date -Format "yyyy-MM-dd-HHmmss"
pyright . | Out-File "dev-docs/code-assessments/pyright-output-$timestamp.txt"

# OR run basedpyright if preferred
basedpyright . | Out-File "dev-docs/code-assessments/basedpyright-output-$timestamp.txt"
```

**For Windows (Command Prompt)**:
```cmd
REM Create dev-docs/code-assessments directory if it doesn't exist
if not exist "dev-docs\code-assessments" mkdir "dev-docs\code-assessments"

REM Run pyright and save output to timestamped file
pyright . > "dev-docs\code-assessments\pyright-output-%date:~10,4%-%date:~4,2%-%date:~7,2%-%time:~0,2%%time:~3,2%%time:~6,2%.txt" 2>&1

REM OR run basedpyright if preferred
basedpyright . > "dev-docs\code-assessments\basedpyright-output-%date:~10,4%-%date:~4,2%-%date:~7,2%-%time:~0,2%%time:~3,2%%time:~6,2%.txt" 2>&1
```

**What to look for in output**:
- Type errors that could cause runtime failures
- Missing type annotations that reduce code clarity
- Inconsistent type usage across the codebase
- Import issues with type stubs
- Generic type parameter problems

**Documentation in assessment**:
- Include a brief summary (max 10 lines, 150 words) of type checking results in the final assessment
- Note the filename of the detailed type checking output file
- Highlight any critical type errors that need immediate attention
- Mention overall type health of the codebase
...
- [ ] **For Python repos**: Run pyright/basedpyright and analyze type checking results
...
**For Python repositories only - include this section if pyright/basedpyright was run**

### Type Checking Summary

**Tool Used**: [pyright/basedpyright]  
**Output File**: `dev-docs/code-assessments/[filename].txt`  
**Assessment Date**: YYYY-MM-DD HH:MM:SS

**Summary**: [Brief summary of type checking results - max 10 lines, 150 words]
- Total errors found: X
- Total warnings: Y
- Critical type issues: Z
- Overall type health: [Excellent/Good/Fair/Poor]
- Key issues identified: [List 2-3 most important findings]
...
**Location**: Lines X-Y  
**Type Error**: [Description of the type error]

**Impact**:
[What this type error could cause at runtime]

**Suggested Fix**:
[How to resolve the type issue]
