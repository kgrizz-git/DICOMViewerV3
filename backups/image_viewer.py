...
class ImageViewer(ImageViewerInputMixin, ImageViewerViewMixin, QGraphicsView):
    ...
    # --- Orientation signals (emitted by context menu and View menu orientation actions) ---
    orientation_flip_h_requested = Signal()   # Request horizontal flip for focused viewer
    orientation_flip_v_requested = Signal()   # Request vertical flip for focused viewer
    orientation_rotate_cw_requested = Signal()   # Request 90° CW rotation for focused viewer
    orientation_rotate_ccw_requested = Signal()  # Request 90° CCW rotation for focused viewer
    orientation_rotate_180_requested = Signal()  # Request 180° rotation for focused viewer
    orientation_reset_requested = Signal()    # Request reset to default orientation
    ...
    def set_rotation(self, deg: int) -> None:
        """
        Set rotation in degrees (normalised to 0, 90, 180, or 270) and
        refresh the view transform.
        """
        self._rotation_deg = int(deg) % 360
        self._apply_view_transform()
        self._notify_orientation_changed()
