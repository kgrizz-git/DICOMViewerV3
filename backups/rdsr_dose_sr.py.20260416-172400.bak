...
**Requirements:** ``pydicom`` only (no Qt). Parsing is **not** a full generic SR renderer; unknown
vendor templates may yield partial summaries without error.
...
def _concept_tuple(item: Dataset) -> tuple[str | None, str | None]:
    """Return (CodeValue, CodingSchemeDesignator) from the first Concept Name Code item."""
    cseq = getattr(item, "ConceptNameCodeSequence", None)
    if not cseq:
        return (None, None)
    seq = cast(Sequence[Dataset], cseq)
    if len(seq) == 0:
        return (None, None)
    c0 = seq[0]
