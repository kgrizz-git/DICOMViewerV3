...
**File → Recent** lists recently opened paths. Use **File → Edit Recent List…** to open the editor: reorder entries with **Move Up** / **Move Down**, remove a single entry with right-click → **Remove This Item**, or clear the list. An empty recent menu shows a disabled **No recent files** placeholder.
...
**Where menus overlap:** the [Layouts](USER_GUIDE_LAYOUTS.md), [Shortcuts](USER_GUIDE_SHORTCUTS.md), and [Configuration](CONFIGURATION.md) guides share many View-menu actions (panes, slice sync, slice location lines, overlay settings, overlay font sizing, side panes). When a setting is described in one guide, the others link to it instead of repeating the same prose. Open whichever entry point you usually reach for — each one points to the rest.
...
- **Histogram:** **Tools → Histogram…** (**Ctrl+Shift+H** / **Cmd+Shift+H**), or the image **right-click** menu, opens the non-modal **Pixel Value Histogram** window for the focused pane. Use **Use intensity projection pixels** to match bins to the active AIP/MIP/MinIP slab instead of the single current slice.
- **DICOM File Info:** **Tools → DICOM File Info…** (**Ctrl+I**), or **DICOM File Info…** on the image / navigator context menu, opens the **DICOM File Info** dialog with high-level path and identity fields for the focused instance (not the full tag editor — use **Tools → View/Edit DICOM Tags…** for that).
- **Close All / Fullscreen:** **File → Close All** (**Ctrl+W**) unloads every study from the viewer. **View → Fullscreen** (**F11** / **Ctrl+F**) toggles the main window fullscreen; see also [USER_GUIDE_SHORTCUTS.md](USER_GUIDE_SHORTCUTS.md).
...
These files live in the repository under `user-docs/` and track the **main** branch on [GitHub](https://github.com/kgrizz-git/DICOMViewerV3) for online **Help → Documentation** links. **Compiled releases** may correspond to an **older** tag than `main`; for behavior tied to a specific build, use that version's notes on [GitHub Releases](https://github.com/kgrizz-git/DICOMViewerV3/releases) and, when online, the [CHANGELOG on `main`](https://github.com/kgrizz-git/DICOMViewerV3/blob/main/CHANGELOG.md).

## Further reading (developers / deep dives)

Implementation notes, research, and plans live in the project's developer documentation (the `dev-docs/` tree), maintained separately from these user guides. Online (requires network; for contributors): [PYLINAC integration overview](https://github.com/kgrizz-git/DICOMViewerV3/blob/main/dev-docs/info/PYLINAC_INTEGRATION_OVERVIEW.md).
