...
| Window / Level | **`wl_user_presets`** — list of custom presets; each entry: **`name`** (string), **`modality`** (`CT`, `MR`, …, or **`ANY`**), **`center`** / **`width`** (float), **`is_rescaled`** (bool: `true` = HU/calibrated, `false` = raw pixels). Managed via **Manage W/L Presets…** (W/L menus or **Edit → Settings…**). Built-in modality presets are not stored in config. |
