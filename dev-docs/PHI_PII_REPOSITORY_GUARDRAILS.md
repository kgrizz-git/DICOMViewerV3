...
- Denies runtime artifacts such as pytest captures, application config files, backups, and `.DS_Store`.
- Scans tracked text data, including JSON, CSV/TSV, YAML, INI, XML, HTML, Markdown, and SVG, for local-path and patient-tag indicators.
- Reads XLSX/XLSM cells for the same indicators.
- Recursively reads DICOM metadata, including sequence items, and rejects populated identifier fields unless an explicitly approved synthetic fixture value is used.
...
`scripts/git_hook_privacy_checks.py` examines staged `src/*.py` changes for unsafe logging, raw exception dialogs, patient fields in output, and machine-specific paths. Use `sanitize_message` and `sanitize_exception` from `src/utils/log_sanitizer.py`; do not add `traceback.print_exc()` to application code.
...
The installed pre-commit hook invokes the staged artifact gate, the repository harness, architecture check, agent smoke harness, Ruff, license check, and the privacy/logging gate. The pre-push hook adds type, coverage, complexity, and security checks.
