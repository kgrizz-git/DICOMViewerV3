...
### Active Shortlist (as of 2026-05-17)

Current working name: **MPDV** (Medical Physics DICOM Viewer) — placeholder until decided.

**Clean candidates** (no meaningful conflicts):

| Name | Abbrev | Theme | Conflict notes |
|---|---|---|---|
| **Radivis** | — | Blend of *radius* / *radiology* + *vis* (Latin: sight, vision) | 🔍 No known conflicts. Clean, pronounceable. Somewhat invented — lacks classical precedent. |
| **Dioptera** | — | Anglicized variant of *dioptra*; same optical + measurement meaning | 🔍 "Dioptra" spelling is likely taken; this variant spelling may be clear. Distinctive, searchable, thematically strong. |
| **Eidos** | — | Greek: form, appearance, essence — root of "eidetic"; Plato's theory of Forms | ⚠️ Eidos Interactive (now Eidos Montréal / Eidos Sherbrooke, Square Enix studios) — gaming association, no healthcare conflict. |
| **Dioptrix** | — | *Dioptra* root + Latin *-trix* suffix (cf. matrix, directrix) — gives a more modern/tech feel | 🔍 No known conflicts. Distinctive, pronounceable, clearly related to optics/measurement. Feels slightly more product-like than Dioptera. |
| **Eikon** | — | Greek: image, likeness — root of "icon" | 🔍 No healthcare conflict; Refinitiv Eikon brand now defunct. ⚠️ Pronounced identically to "icon" — word-of-mouth spelling is ambiguous, search discoverability poor. |

**Conflicted but not excluded** (usable with caveats):

| Name | Abbrev | Theme | Conflict notes |
|---|---|---|---|
| **Argus** | — | Greek myth: the hundred-eyed all-seeing giant | ⚠️ Oracle Argus Safety (pharmacovigilance, widely used in pharma); Argus II (FDA-approved retinal implant). Both known to medical professionals. |
| **Meridian** | — | Lines of measurement; precise navigation | ⚠️ Meridian Teleradiology — direct domain overlap. Fallback only. |
| **Quanta** | — | Units of energy; medical physics / radiation | ⚠️ Quanta System (Italian medical laser company, 30,000+ hospital installs). Fallback only. |

**Excluded — conflicts too strong:**

| Name | Reason |
|---|---|
| **Dioptra** | ⚠️→❌ Likely taken — "Dioptera" variant kept as active candidate instead. |
| **Axion** | ⚠️→❌ Likely taken — verify before reconsidering. |
| **Lynx** | ❌ Lynx Medical Systems (healthcare documentation, 350+ hospitals). |
| **Praxis** | ❌ Praxis EMR — strongly associated in healthcare. |
| **Apex** | ❌ Very crowded in healthcare software. |

### Naming notes

- **Argus**: mythologically resonant (all-seeing), short, memorable. Oracle Argus Safety conflict is in pharmacovigilance case management — different product category. Low practical risk while free/open-source.
- **Eikon**: clean conflict profile, directly means "image." Discoverability is the main weakness — homophone of "icon" means word-of-mouth spelling is ambiguous.
- **Dioptera** / **Dioptrix**: both variants of *dioptra*. Dioptera is the more classical-sounding anglicization; Dioptrix adds the Latin *-trix* suffix (matrix, directrix) for a more modern/product feel. Either may be clear since the "Dioptra" spelling is what appears taken.
- **Eidos**: directly means "form/appearance" in Greek — highly relevant for an imaging tool. The Eidos Interactive gaming brand (now Square Enix studios) is the main association; no healthcare conflict, but the gaming connotation may feel off in a clinical setting.
- **Radivis**: invented compound; no conflicts found; easy to explain ("radiology + vision"). Lacks the depth of the classical names.
- **Meridian** / **Quanta**: both have meaningful healthcare conflicts; kept as fallbacks only.
- All names should be verified against `.com` / `.io` domain availability and trademark databases (USPTO, EUIPO) before finalising.
