...
**Bottom line on SmartScreen:** any signing path triggers a first-launch warning until publisher-certificate and file-hash reputation accumulate — potentially weeks and hundreds of clean downloads per Microsoft. Publishing through the **Microsoft Store** avoids even that initial warning for new publishers (Microsoft re-signs Store apps with its own certificate), but it's not the only way to reach a no-warning experience: once your certificate and file hashes have built sufficient positive reputation, warnings stop — and on Windows 11 Smart App Control (which can *block* unsigned binaries) also respects established reputation. See [SmartScreen reputation for Windows app developers](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/smartscreen-reputation).
...
- **Code signing scope narrows** (sign one file instead of hundreds — a genuine pro for onefile). Note UPX interactions are unchanged: pack first, sign after, modify never (see the signing section above).
