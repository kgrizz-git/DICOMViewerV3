...
> **Update 2026-06-14 (decoder spike):** a quick pydicom-3 spike (force-installing `pydicom==3.0.2` past pylinac's metadata cap) found **pylinac 3.43.2 runs fine under pydicom 3** — `import pylinac`/`ACRCT`/`ACRMRILarge` succeed and **CatPhan504 (CT) + QuartDVT (MRI)** analyses give **identical** results on pydicom 2.4.5 vs 3.0.2. So pylinac's `pydicom<3` cap is **conservative**, and mitigation (2) may not require waiting for upstream — a controlled pydicom-3 bump (overriding the cap) looks viable pending full ACR QA re-verification. This unblocks **both highdicom and the BSD `imagecodecs` decoder**. Details: [`PYLIBJPEG_ALTERNATIVES_AND_DICOM_DECODER_STRATEGY.md`](PYLIBJPEG_ALTERNATIVES_AND_DICOM_DECODER_STRATEGY.md); TO_DO (Maintenance).
