...
1. [PYLINAC_ACR_FULL_METRICS_EXPORT_AND_MRI_BATCH_PLAN.md](../plans/completed/PYLINAC_ACR_FULL_METRICS_EXPORT_AND_MRI_BATCH_PLAN.md) — **completed implementation** (Phases 0–6 export, batch, SNR).
...
- [ACR phantom QA metrics and pylinac gaps](ACR_PHANTOM_QA_METRICS_AND_PYLINAC_GAPS.md) — **ACR accreditation + QC manual** metrics, links, pylinac gaps (**SNR** harvested in Phase 6; deferred resolution reads).
...
**`requirements.txt`** pins **`pylinac==3.43.2`** and lists **`pypdf>=4.0.0`** as an explicit direct dependency (with a short comment in that file) for merging per-run pylinac PDFs into the combined compare report—see `dev-docs/plans/completed/MRI_COMPARE_COMBINED_PDF_PLAN.md`. The **`pylinac==3.43.2`** pin is the **current** upstream release used with this project’s ACR CT / ACR MRI Large integration (constructors, `analyze()`, optional `check_uid`, and the relaxed scan-extent subclasses); follow **`dev-docs/plans/completed/DEPENDENCY_BUMP_VERIFICATION_PLAN.md`** after each bump. Other pylinac versions may work but should be treated as **unverified** until that plan (and targeted QA) is completed; update this subsection and the comment in `requirements.txt` whenever the pin changes.

`reportlab` (a pylinac transitive dependency) is also used directly by the viewer to build the compare-mode summary page.

### pydicom version compatibility (pylinac caps `pydicom<3`)

**The app stays on pydicom 2.x because pylinac caps it.** `requirements.txt` allows `pydicom>=2.4.0,<3`; the `<3` ceiling comes from **`pylinac 3.43.2` → `pydicom<3,>=2.0`** (and the **latest** pylinac, 3.44.0 PyPI / 3.45.0 dev, **still** caps `<3` as of 2026-06-14). This same cap blocks **highdicom** and the BSD **imagecodecs** decoder (see [`HIGHDICOM_OVERVIEW.md`](HIGHDICOM_OVERVIEW.md) §4a, [decoder strategy](PYLIBJPEG_ALTERNATIVES_AND_DICOM_DECODER_STRATEGY.md)).

**The cap is conservative — pylinac 3.43.2 actually runs fine on pydicom 3.0.2.** Spike (2026-06-14): the only code-level incompatibility is `pylinac/core/image.py` importing `pydicom.pixel_data_handlers` (`apply_rescale`), reorganized into `pydicom.pixels` in v3 — but pydicom 3.0.2 still ships that shim. Validation forced `pydicom==3.0.2` past the cap and compared full results vs 2.4.5:

- **ACR CT** + **ACR MRI Large** (app modules via `run_acr_ct_analysis` / `run_acr_mri_large_analysis`): **byte-identical** `raw_pylinac` (module ROIs, uniformity, CNR, MTF) except the analysis timestamp.
- **CatPhan504, QuartDVT**: identical.
- **Nuclear: 8/8** tests byte-identical (SimpleSensitivity multi-file not run).

**Implication:** a future coordinated **pydicom 3 + pylinac** bump (override the metadata cap in a controlled PyInstaller build, then re-run [`DEPENDENCY_BUMP_VERIFICATION_PLAN`](../plans/completed/DEPENDENCY_BUMP_VERIFICATION_PLAN.md) with real ACR data + the pylinac suite) looks viable and would unblock imagecodecs + highdicom. Tracked in `TO_DO.md` (Maintenance).

**Related pylinac runtime quirks** (scikit-image deprecations; nuclear All-NaN contrast): [`PYLINAC_FLEXIBILITY_AND_WORKAROUNDS.md`](PYLINAC_FLEXIBILITY_AND_WORKAROUNDS.md) §7.
...
| **ACR MRI Large** | **`ACRMRILargeForViewer`** (alias **`ACRMRILargeRelaxedExtent`**) — same runner/worker pattern and **relaxed image extent** as CT by default; **Tools → ACR MRI…**; dialog **`acr_mri_qa_dialog.py`** for echo (default **highest** EchoNumber), low-contrast kwargs (persisted), scan tolerance (hidden when vanilla), **`check_uid`**, **`origin_slice`**. **Vanilla pylinac** uses stock **`ACRMRILarge`**. Viewer harvests uncorrected **`metrics.mri_snr`** from the uniformity Center ROI / freq-encode ghost ROI σ (no 0.655). |
| **Nuclear (NM/SPECT)** | **`pylinac.nuclear`** — **all nine tests**: **`PlanarUniformity`**, **`FourBarResolution`**, **`QuadrantResolution`**, **`CenterOfRotation`**, **`TomographicResolution`**, **`MaxCountRate`**, **`TomographicUniformity`**, **`TomographicContrast`**, **`SimpleSensitivity`** (flat tests share `_run_flat_nuclear_analysis`; SPECT/dynamic tests use all frames — no first-frame warning; TomographicContrast normalizes a per-sphere nested dict + `uniformity_baseline`; **SimpleSensitivity** takes an optional 2nd/background file + a `Nuclide` enum + required activity, and has no `plot()` so Save Figure is gated by `is_plottable`) via **`src/qa/pylinac_nuclear.py`** (`run_nuclear_analysis` dispatcher; re-exported through `pylinac_runner`; worker routes any `analysis_type` in `NUCLEAR_ANALYSIS_TYPES`). **Single DICOM file** input (focused image or file picker); per-class **`NuclearOptions`** payloads nested on `QARequest` (`PlanarUniformityOptions` / `FourBarResolutionOptions` / `QuadrantResolutionOptions`, a tagged union keyed on `analysis_class`, each with its own `analyze_kwargs` / `is_pylinac_default`); **NM modality preflight**. Three result shapes are normalized: PlanarUniformity's frame-keyed `results_data()` → **`metrics.frames`**; FourBar's **flat** 8-float result → **`metrics.results`**; Quadrant's **per-quadrant** nested dict → **`metrics.quadrants`** (resolution tests are first-frame-only, with a multi-frame warning). All store verbatim **`raw_pylinac`**. Provenance via **`build_nuclear_analysis_profile`** (no ACR-only scan-extent/relaxed-extent keys). **Tools → Automated QA → Nuclear Medicine QC (pylinac)…**; options dialog **`gui/dialogs/nuclear_qa_dialog.py`** swaps per-test parameter pages (Quadrant: 4 bar widths + ROI geometry); result dialog **`nuclear_result_dialog.py`** renders a per-frame, per-quadrant, or Metric/Value table with JSON/CSV/PNG export. Figure render is class-dispatched in **`pylinac_nuclear_plots.py`** (main thread). Plans: [PlanarUniformity](../plans/supporting/PYLINAC_NUCLEAR_MEDICINE_MODULE_INTEGRATION_PLAN.md), [FourBarResolution](../plans/supporting/PYLINAC_NUCLEAR_FOURBAR_RESOLUTION_PLAN.md). |
| **ACR CT CNR / batch / XLSX** | **CNR intermediates:** `_extract_low_contrast_cnr_details` (`src/qa/pylinac_acr_ct.py`) harvests object ROI mean, background mean/σ, and module CNR (dict-valued `low_contrast_module.rois` / `.background_rois`; `cnr()` is a method) into **`metrics.low_contrast_cnr`**; surfaced in the result dialog. **Canonical flatten:** `src/qa/qa_result_flatten.py` (`build_metric_rows` / `build_tabular_run`) walks `raw_pylinac` and overlays curated `result.metrics` entries (metrics wins on key collision); shared by CSV, XLSX Detail, and batch CSV. **Batch CT:** **`QACTBatchWorker`** (`src/qa/worker.py`) runs multiple CT series **serially** (per-series error isolation, cooperative cancel, worker-owned image + module-image temp dirs) into **`CTBatchResult`**; selection dialog `ct_batch_select_dialog.py`, summary dialog `ct_batch_result_dialog.py` (Export CSV / XLSX / JSON), **`acr_ct_batch_requested`** signal. **XLSX:** **`build_qa_workbook`** (`src/qa/qa_xlsx_export.py`, Qt-free) — **Summary** (modality-aware key columns: PIU, PSG, LC score, MTF@50% row/col, slice thickness/shift on MRI, uncorrected **MRI SNR**; best-effort, blank where a metric does not apply), **Detail** (full flatten via `build_metric_rows`), **Images** (per-module PNGs from `QAResult.analyzed_module_images` when `embed_module_images_in_xlsx` is on; falls back to legacy composite `analyzed_image_path`; skipped with Summary note when toggle off / no Pillow). **MRI batch:** **`QAMRIBatchWorker`** + **`ACRMBatchResult`** (mirrors `CTBatchResult`; **not** compare-mode `MRIBatchResult`); `acr_mri_series_selection_dialog.py` (MR series filter + Add folder), `mri_batch_result_dialog.py` (Export CSV / XLSX / JSON), `acr_mri_batch_requested` signal; flow in `src/gui/qa_mri_batch_flow.py` (keeps `QAAppFacade` unchanged). Plan: [PYLINAC_ACR_FULL_METRICS_EXPORT_AND_MRI_BATCH_PLAN.md](../plans/completed/PYLINAC_ACR_FULL_METRICS_EXPORT_AND_MRI_BATCH_PLAN.md). |
...
**Nuclear QA specifics (JSON schema and figures)**

- **JSON schema:** nuclear runs reuse single-run **`schema_version` "1.1"**; consumers discriminate on **`run.analysis_type`** (`nuclear_planar_uniformity`) plus a convenience **`run.nuclear_analysis_class`** added only for nuclear payloads. Per-frame results live under **`metrics.frames["Frame N"]`**; the full pylinac payload is under **`raw_pylinac`**. Nuclear runs were intentionally **not** bumped. **Note:** the **ACR CT** single-run document has since moved to **`schema_version` "1.3"** — for CT, `raw_pylinac` is now the structured `results_data(as_dict=True)` dict (opaque, pylinac-version-dependent passthrough) rather than a stringified blob (see the ACR CT CNR/XLSX row below and CHANGELOG). "1.2" remains the MRI compare document.
- **Plots/figures (main-thread render):** `PlanarUniformity` exposes `plot()` (one matplotlib `Figure` per frame) but **no `publish_pdf`**. Because the QA worker runs on a `QThread` and the app uses a **Qt matplotlib backend** (`tools/histogram_widget.py` → `FigureCanvasQTAgg`), figures must **not** be built on the worker thread. The **Save Figure (PNG)…** button in the result dialog therefore calls `qa.pylinac_nuclear_plots.render_nuclear_figures` on the **main thread**, which re-runs the (fast) analysis and saves one PNG per frame (`…_Frame_<key>.png` for multi-frame). The worker still returns only **numeric metrics + JSON**; no figure work happens off the main thread.
...
- **Deep GUI integration**: showing pylinac **annotated images or ROIs** as **overlays** in the main image viewer; **site-configurable pass/fail thresholds** in the viewer (beyond exporting metrics and optional PDF). **Batch runs are shipped** for **ACR CT** and **ACR MRI** (serial multi-series via `QACTBatchWorker` / `QAMRIBatchWorker` — see the ACR CT CNR / batch / XLSX row above); queued/parallel runs remain future work.
