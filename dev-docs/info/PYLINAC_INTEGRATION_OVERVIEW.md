...
| **ACR CT CNR / batch / XLSX** | **CNR intermediates:** `_extract_low_contrast_cnr_details` (`src/qa/pylinac_acr_ct.py`) harvests object ROI mean, background mean/σ, and module CNR (dict-valued `low_contrast_module.rois` / `.background_rois`; `cnr()` is a method) into **`metrics.low_contrast_cnr`**; surfaced in the result dialog. **Batch:** **`QACTBatchWorker`** (`src/qa/worker.py`) runs multiple CT series **serially** (per-series error isolation, cooperative cancel, worker-owned image temp dir) into **`CTBatchResult`**; selection dialog `ct_batch_select_dialog.py`, summary dialog `ct_batch_result_dialog.py`, **`acr_ct_batch_requested`** signal. **XLSX:** **`build_qa_workbook`** (`src/qa/qa_xlsx_export.py`, Qt-free; Summary/Detail/Images with embedded analyzed image via transient `QARequest.analyzed_image_out_path` / `QAResult.analyzed_image_path`). Plan: [PYLINAC_CT_CNR_BATCH_XLSX_PLAN.md](../plans/PYLINAC_CT_CNR_BATCH_XLSX_PLAN.md). |
...
- **Deep GUI integration**: showing pylinac **annotated images or ROIs** as **overlays** in the main image viewer; **site-configurable pass/fail thresholds** in the viewer (beyond exporting metrics and optional PDF). **Batch runs are partially shipped** for **ACR CT** (serial multi-series via `QACTBatchWorker` — see the ACR CT CNR / batch / XLSX row above); queued/parallel runs and batch for other modalities remain future work.
