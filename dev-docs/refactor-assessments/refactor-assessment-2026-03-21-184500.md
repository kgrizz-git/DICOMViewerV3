...
**Proposed Structure**:
- New/expanded module: `src/core/app_signal_wiring.py`
  - Move all `_connect_*` and focused signal helper methods.
- Keep in `main.py`:
  - Lifecycle ordering and high-level orchestration.

**Migration Strategy**:
1. Move signal connect/disconnect logic in cohesive groups.
2. Keep method names stable through wrapper delegation in `DICOMViewerApp`.
3. Verify no signal ordering regression through existing startup tests + focused smoke tests.

**Benefits**:
- Reduces `main.py` by several hundred lines.
- Makes startup event flow testable without full app launch.
- Lowers merge conflict risk in core app class.
...
- **Recommendation**: 5/5 - High-value split.
...
7. Continue decomposition of `src/main.py` by moving signal wiring and dialog command handlers (Score: 4.00/5)
...
The following files exceed 750 lines but are potentially acceptable in current form if content is intentionally embedded and low-churn:

- **`src/gui/main_window_theme.py`** (826 lines): mostly large stylesheet payload; can remain until theme package extraction is scheduled.
- **`src/gui/dialogs/quick_start_guide_dialog.py`** (835 lines): much of the size is documentation content, not branching logic.
- **`src/gui/dialogs/fusion_technical_doc_dialog.py`** (811 lines): size is driven by embedded technical documentation text.
