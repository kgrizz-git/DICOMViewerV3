...
**Relevant files**
- `src/gui/metadata_panel.py` - Add privacy edit guard, active edit-dialog tracking, and forced-close-on-privacy helper.
- `src/gui/dialogs/tag_viewer_dialog.py` - Add privacy edit guard in _edit_tag_item, active dialog tracking, and forced-close helper.
- `src/gui/dialogs/tag_edit_dialog.py` - No required behavior change; keep as generic editor unless a tiny API hook is needed for robust cleanup callbacks.
- `src/gui/dialog_coordinator.py` - Add privacy update fan-out for open tag viewer/editor dialog state.
- `src/core/privacy_controller.py` - Include dialog coordinator in privacy propagation sequence.
- `src/main.py` - Pass dialog coordinator dependency into PrivacyController construction.
- `src/utils/dicom_utils.py` - Reuse is_patient_tag(tag_str) helper for lock decisions.
