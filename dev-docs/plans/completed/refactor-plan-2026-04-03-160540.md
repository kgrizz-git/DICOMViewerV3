...
## Progress snapshot (verified) — 2026-04-05 (Phase 3 row updated 2026-04-06; Phase 4 complete 2026-04-07; Phase 5A–5D slices through 2026-04-08)
...
**Phase 4a (projection) landed (2026-04-07):** **`src/core/projection_app_facade.py`** — `ProjectionAppFacade` owns former **`DICOMViewerApp`** bodies for **`_sync_intensity_projection_widget_from_mpr_data`**, **`_on_projection_enabled_changed`**, **`_on_projection_type_changed`**, **`_on_projection_slice_count_changed`**. **`main.py`** keeps one-line delegates; **`subwindow_lifecycle_controller.py`** connections unchanged.

**Phase 4b (QA / pylinac) landed (2026-04-07):** **`src/core/qa_app_facade.py`** — preflight, **`QAAnalysisWorker`** / **`QABatchWorker`** orchestration, extent retry, ACR CT/MRI entry flows, compare results dialog, **`_open_path_in_system_viewer`**, single-run and compare JSON export. **`app_signal_wiring`** still connects **`acr_*_phantom_requested`** to app methods that delegate.

**Phase 4c (export entry / paths) landed (2026-04-07):** **`src/core/export_app_facade.py`** — **`resolve_focused_series_ordered_paths`**, **`prompt_save_path`**, **`open_export_roi_statistics`**, **`open_export`** (via **`dialog_action_handlers.open_export(app)`**), **`open_export_screenshots`**. App retains **`_prompt_save_path`** / **`_resolve_focused_series_ordered_paths`** so **`QAAppFacade`** call sites need no edits.

**Phase 5A (fusion — first slice) landed (2026-04-07):** **`src/core/fusion_handler_io.py`** — pure DICOM spatial readers, sorted slice location list, IPP→pixel translation, 2D linear blend; **`FusionHandler`** delegates and keeps caches / 3D resampling policy. Duplicate-slice detection no longer uses a temporary cache key.

**Phase 5B (ROI clipboard persistence — first slice) landed (2026-04-07):** **`src/tools/roi_persistence.py`** — `serialize_roi_for_clipboard` / `serialize_rois_for_clipboard`; **`AnnotationClipboard`** delegates ROI list building; **`roi_manager`** module doc points to **`roi_persistence`** (serialization lived in **`annotation_clipboard`** historically, not on **`ROIManager`**).

**Phase 5C (MPR geometry — first slice) landed (2026-04-07):** **`src/core/mpr_geometry.py`** — `MprOutputGrid` + **`compute_mpr_output_grid`**, **`stack_positions_along_normal`**, **`standard_slice_planes_lps`**; **`MprBuilderWorker`** / **`MprBuilder.standard_planes`** delegate; **no SimpleITK** inside **`mpr_geometry`**.

**Phase 5D (metadata panel — first slice) landed (2026-04-08):** **`src/gui/metadata_table_model.py`** — **`MetadataItemDelegate`** + pure **`filter_metadata_tags_by_search`**, **`group_metadata_tags_sorted`**, **`format_metadata_value_for_tree`**; **`metadata_panel.py`** keeps **`QTreeWidget`** UI (no **`QAbstractTableModel`** in codebase; plan “table” = columnar tree).
...
- [x] **Phase 4** — **done** (2026-04-07): **`projection_app_facade.py`**, **`qa_app_facade.py`**, **`export_app_facade.py`**; re-verify after large merges.
- [ ] **Phase 5** — **5A–5D first slices done**; track **5E** still open.
...
- [x] Manual smoke: zoom/pan, tools, context menu, multi-window layout (recommended for any UI-touched release).
...
- [x] Discovery: all three façades under `src/core/` (2026-04-07).
- [x] Extract **4a** — `projection_app_facade.py`; `DICOMViewerApp` delegates; lifecycle wiring unchanged (2026-04-07).
- [x] Extract **4b** — `qa_app_facade.py`; ACR QA/compare inventory; **`app_signal_wiring`** unchanged (2026-04-07).
- [x] Extract **4c** — `export_app_facade.py`; export / save-path / focused-series inventory (2026-04-07).
- [x] Run tests (full suite green 348 passed, 2026-04-07).
- [x] Manual smoke: projection UI, QA/export/screenshot flows when touching those areas. 
- [x] **AGENTS.md** — `core/` map lists all three façades (2026-04-07).
...
- [x] Discovery: **5A** — `fusion_handler.py` spatial + 2D blend helpers (2026-04-07).
- [x] Extract **5A first slice** — `src/core/fusion_handler_io.py` (no Qt); `FusionHandler` delegates (2026-04-07).
- [x] Wire imports; **`FusionHandler`** public API unchanged.
- [x] Discovery: **5B** — ROI clipboard dict layout (`annotation_clipboard` + `paste_roi`) (2026-04-07).
- [x] Extract **5B first slice** — `src/tools/roi_persistence.py`; **`AnnotationClipboard`** uses it; **`roi_manager`** doc link (2026-04-07).
- [x] **AGENTS.md** — **`tools/roi_persistence.py`**, **`core/mpr_geometry.py`**, **`gui/metadata_table_model.py`**, **`utils/undo_redo_tag_commands.py`**, **`gui/dialogs/mri_compare_result_dialog.py`** (2026-04-08).
- [x] Discovery / extract **5C** — **`src/core/mpr_geometry.py`**; **`mpr_builder`** delegates (2026-04-07).
- [x] Extract **5D** — **`src/gui/metadata_table_model.py`**; **`metadata_panel`** wires UI only (2026-04-08).
- [x] Extract **5E** — **`src/utils/undo_redo_tag_commands.py`** (`TagEditCommand`); **`src/gui/dialogs/mri_compare_result_dialog.py`**; **`qa_app_facade`** delegates (2026-04-08).
- [x] Run full suite after **5D** — **357 passed** (2026-04-08).
- [x] Run full suite after **5E** — **360 passed** (2026-04-08).

#### Manual smoke — Phase 5 (run the rows for tracks you touch; full matrix before a release candidate)

| Track | Manual smoke (expected: no regression vs prior behavior) |
|-------|------------------------------------------------------------|
| **5A Fusion** | Load study with PET+CT (or two series with same FoR). Open **Fusion**, pick base + overlay, enable fusion. **Toggle** opacity/threshold/colormap. **Swap** overlay series once. Exercise **fast vs high-accuracy** resampling if available. |
| **5B ROI / annotations** | Draw **rectangle + ellipse** ROI on a slice. **Select** ROI, **Copy** annotations, change slice (or same slice), **Paste**; confirm shape/position/statistics overlay sane. **Undo/redo** one ROI operation if enabled. **Export ROI statistics** dialog still opens (if tied to ROI data). |
| **5C MPR / lifecycle** | Open **MPR** on a multi-slice CT. **Scroll** slabs, **change orientation** (axial/sag/cor if applicable), **return** to 2D. **Combine slices / projection** if used in same session. |
| **5D Metadata / DICOM** | Open **metadata / tag** panel. **Scroll** long tag list. **Edit** a tag (if allowed), **undo/redo**. **Toggle privacy view**; confirm burn-in / hidden PHI behavior unchanged. **Export DICOM Tags** dialog: open, pick a few tags, export small sample. |
| **5E Undo / pylinac** | **Edit metadata tag** → **undo** → **redo** (order must match before refactor). Run **ACR CT or MRI QA** once; open **result dialog**; **export JSON** if touched. If MRI **compare** UI moved, run compare mode and confirm PDF/JSON paths. |
...
| **5B ROI / annotations** | Extract **serialization / JSON or pickle helpers** from `src/tools/roi_manager.py` (or `annotation_manager.py`) to `src/tools/roi_persistence.py` (or shared `annotation_persistence.py`) — one manager only in the first PR. | Load/save or copy/paste behavior unchanged for that manager; **`roi_persistence` wired from `annotation_clipboard`** (2026-04-07 first slice); ROI-related tests green. |
| **5C MPR / lifecycle** | Extract **spacing/key/frame math** from `src/core/mpr_builder.py` into `src/core/mpr_geometry.py` (functions only); `MprBuilder` calls helpers. | MPR open/rotate/scroll parity; **`mpr_geometry`** landed 2026-04-07 (grid + standard planes + stack positions); tests green; manual: MPR row in smoke table. |
| **5D Metadata / DICOM / dialogs** | Split **`src/gui/metadata_panel.py`** table model into `src/gui/metadata_table_model.py` (QAbstractTableModel subclass + delegates) with `metadata_panel.py` wiring only. | **2026-04-08:** delegate + pure tag helpers in **`metadata_table_model`** (panel remains **`QTreeWidget`**); scroll/edit/history/privacy unchanged; **`tests/test_metadata_table_model.py`**. |
| **5E Undo / pylinac** | Extract **one command family** from `src/utils/undo_redo.py` (e.g. tag-edit commands only) into `src/utils/undo_redo_tag_commands.py` re-exported from `undo_redo.py` for compatibility. **Alternatively** (lower coupling): isolate **MRI compare dialog UI** from `main.py` / runner into `src/gui/dialogs/mri_compare_result_dialog.py` if undo is too hot. | **2026-04-08:** both slices — **`TagEditCommand`** + **`create_mri_compare_result_dialog`**; `undo_redo.py` end-import re-export; **`tests/test_undo_redo_tag_commands.py`**; full suite **360** passed. |
...
- [PARALLEL_WORKSTREAM_OWNERSHIP_PLAN.md](../supporting/PARALLEL_WORKSTREAM_OWNERSHIP_PLAN.md)
