...
## Progress snapshot (verified) — 2026-04-06
...
- [ ] **Phase 2** — **2a** only `src/core/slice_display_manager.py` (no `slice_display_lut.py` / `slice_display_pixels.py` split observed); **2b** only `src/gui/series_navigator.py` (no `series_navigator_model.py` / `series_navigator_view.py` split observed).
- [ ] **Phase 3** — `src/gui/image_viewer.py` remains monolithic (no `image_viewer_view.py` / related split files observed).
...
- `glob **/slice_display_lut.py **/slice_display_pixels.py` (expect absent until split lands).
...
- [ ] Extract module(s): add LUT and/or pixel helper module(s); reduce `slice_display_manager.py` surface thoughtfully.
- [ ] Wire imports; preserve `SliceDisplayManager` public API for callers.
- [ ] Run tests.
- [ ] Manual smoke: W/L, cine, representative series (scope to display path).
- [ ] Optional **AGENTS.md** if core display map changes.
...
- `glob **/series_navigator_model.py **/series_navigator_view.py` (expect absent until split lands).

### Checklist

- [x] Discovery: only `src/gui/series_navigator.py` observed; no separate model/view files (2026-04-05).
- [ ] Extract module(s): `series_navigator_model.py` / `series_navigator_view.py` (or equivalent split).
- [ ] Wire imports and signals; keep external behavior stable.
- [ ] Run tests.
- [ ] Manual smoke: series bar, selection sync across windows (if applicable).
- [ ] Optional **AGENTS.md** if navigator structure is documented for contributors.
...
**Rule:** Do **not** treat the sections below as “ready to implement” until the listed expansion work exists (here or in a **dated child plan** under `dev-docs/plans/`).

| Phase | Expand detail before implementing |
|-------|-------------------------------------|
| **3 — ImageViewer** | Add a child plan or appendix: **first-PR scope** (concrete methods/classes or regions to move in step 3a), import boundaries, and **stacked PR order** agreed with reviewers. Optionally spike read-only notes from `rg`/outline. Large-file refactors merge poorly without this. |
| **4 — `main.py` façades** | List **which `DICOMViewerApp` method groups** land in each façade (names or `rg` anchors), order **4a → 4b → 4c**, and any **`app_signal_wiring` / handler** touch points. Re-run or supplement a **refactor assessment** if `main.py` has drifted since the original inventory. |
| **5 — Domain tracks** | Per chosen track (5A–5E), add a **first merge-sized slice** (single primary file or one extraction) and “done when” bullets so parallel owners do not collide on the same coordinator. |
| **6 — Opportunistic** | When picking a file from the Phase 6 table, add **2–5 lines** to the active PR/plan: what helper moves, why, and smoke scope — no need to pre-write the whole table. |
...
- `glob **/image_viewer_*.py` (expect absent while monolithic).

### Checklist

- [x] Discovery: `src/gui/image_viewer.py` still monolithic; no `image_viewer_view.py` etc. observed (2026-04-05).
- [ ] Extract module(s): view/scene, input, context menu per sub-steps above.
- [ ] Wire imports: `sub_window_container.py`, coordinators, `main.py` construction as needed.
- [ ] Run tests (including any `ImageViewer`-referencing tests).
- [ ] Manual smoke: zoom/pan, tools, context menu, multi-window layout.
- [ ] Optional **AGENTS.md** when module layout stabilizes.
...
- **Later phases (3–6):** Before substantial implementation, complete the **detail expansion** steps in the section **Detail expansion before implementation (Phases 3–6)** (child plan, method groups, or first-slice scope as appropriate).
