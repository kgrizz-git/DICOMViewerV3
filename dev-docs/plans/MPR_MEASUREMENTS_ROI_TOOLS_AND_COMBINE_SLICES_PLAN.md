...
1. MPR dialog supports slab combine controls (`None|MIP|MinIP|AIP`) and slab thickness.
2. MprBuilder applies slab combine so the displayed slice arrays are already combined.
3. MPR cache keys include slab combine parameters and old caches are invalidated safely.
4. MPR mode no longer hard-disables ROI/measurement/WL ROI tool activation.
5. ROI statistics on MPR use the displayed MPR slice arrays and use MPR output pixel spacing.
6. MeasurementTool uses MPR output pixel spacing on MPR subwindows.
7. QA confirms ROI stats + auto-WL match the displayed MPR pixels for all slab modes.
