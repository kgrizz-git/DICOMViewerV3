...
## 3. Large-File Warning (confirm / cancel before load)

**Status (2026-06-04):** Shipped at **25 MB** threshold with **Cancel** / **Continue** confirmation before any load pipeline starts (`FileDialog.confirm_large_files`, `FileOperationsHandler._check_large_files` → bool; all call sites in `file_operations_handler.py`). Separate threshold bump to **50 MB** remains optional (§3.1 below).

### 3.0 Confirm / cancel UX (implemented)

- When one or more paths exceed `LARGE_FILE_WARNING_THRESHOLD_MB` (25), show a modal **Large Files** dialog **before** `run_load_pipeline_async` / pixel read.
- Buttons: **Continue** (proceed) and **Cancel** (abort; no navigator update).
- List file basenames and sizes (largest first); folder scan and drag-drop use the same check on the scanned path list.
- Dialog uses `WindowStaysOnTopHint` initially (same pattern as other `FileDialog` messages).
