...
3. **[P1] Documentation audit slice 2a — 8 user-facing rows against the running UI.** Assess DOC-01, 06, 07, 08, 09, 10, 18, 19 per each row's verification recipe in [`DOCUMENTATION_INVENTORY.md`](DOCUMENTATION_INVENTORY.md) (TRIAGE-038). **Gates:** distributing the offline doc bundle (C2 contingency). Platform work is done — [user docs platform plan](plans/completed/USER_DOCS_PLATFORM_AND_SYNC_HARNESS_PLAN.md) archived (A+B+C0+CZ+C2; Zensical adopted).
