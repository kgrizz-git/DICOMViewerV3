...
>
> **Manual smoke rule:** while [Manual Smoke Checks](#manual-smoke-checks) has
> open `[ ]` items, slot **1** is always that batch — do not replace it with
> other work.
...
> **Scope:** Human verification after merged implementation work when automated tests are green but eyes are still required. **Not** for open implementation tasks — those stay in their feature section until code ships.
>
> **Workflow:**
> 1. When a plan is complete except for manual smoke and its work is merged, archive the plan to [`plans/completed/`](plans/completed/) and move the remaining checks here (one line per distinct check).
> 2. Run using [`dev-docs/orchestration/AGENT_SMOKE.md`](orchestration/AGENT_SMOKE.md) as the general prelude; add plan-specific steps on each item below.
...
- [ ] **[P2]** **Reconcile the Unpushed Edits sheet:** file each open item as a concrete dated entry below, then delete this pointer. No open-ended "see X" placeholders.
- [ ] **[P2]** **Collect & curate a demo/test DICOM image set** for pylinac QA and decoder testing. Sources: own data + public demo/test data (pylinac demo bucket, `pydicom-data`). Bundle **small, non-PHI** samples where licensing allows; record provenance. **Status 2026-08-29:** de-identified ACR CT (25) + ACR MRI (45) instances are tracked under `sample-phantom-data-committed/deid-phantoms/` (PS3.15 Standard share; pixel review 2026-08-29). Remaining: provenance URLs for additional public/demo sets; keep raw local copies gitignored.
- [ ] **[P1]** **Maintain HoundDog.ai as a local-only, non-blocking privacy code/data-flow scanner.** HoundDog is now adopted (PRIV-T1 advisory wrapper shipped) and runs **local-only** on `main` push after blocking gates pass. It remains disconnected from accounts, uploads, hosted processing, repository integrations, and CI, and its report never reveals a matched value. The recurring task is verifying this posture holds (no source or scan result uploaded), not initial evaluation. Do not relax the local-only restriction without an explicit user decision. See [HoundDog evaluation constraints](info/LOCAL_PHI_PII_DETECTION_MODEL_OPTIONS.md#hounddogai-high-priority-source-code-candidate).
- [ ] **[P1]** **Maintain local OCR for burned-in PHI and DICOM pixel review.** OCR tooling is adopted (Tesseract, Presidio, `dicom-phi-scan`/EasyOCR wired via `privacy_tool_review.py` + conditional staged reviews in an isolated PHI-tools venv; PRIV-T4 shipped). The recurring task is operating it and periodically re-auditing engine-vs-model provenance, model downloads, licensing, resource use, and safe report behaviour using only temporary, wholly synthetic DICOM, image, PDF, and Office/OpenDocument-package fixtures outside the repository; scan DICOM pixels independently from metadata. Keep the result local-only and warning-only; report only path, category, confidence, and safe position metadata—never detected text or image crops. Do not trust `BurnedInAnnotation=NO`, OCR silence, or an AI result to skip human review. GitHub’s public secret-scanning/public-monitoring features audit credentials, not PII/PHI, so they are not a substitute. See [DICOM PHI Scanner evaluation constraints](info/LOCAL_PHI_PII_DETECTION_MODEL_OPTIONS.md#dicom-phi-scanner-useful-reference-and-synthetic-only-spike).
...
- [ ] **[P2]** **Privacy / PHI / PII leakage hardening (tail).** Blocking gates, hooks, and CI privacy jobs are **operational** (artifact admission, staged privacy checks, Gitleaks history, redacted summaries). Optional tail: consent/settings **manual smoke** (PRIV-V5), doc drift (PRIV-I4/I5), and historical sink-migration checkboxes in the plan — **not** Next-up priority. Remote-history gate **G6** is superseded (repo continued on `main`; recreation runbook kept as contingency only). Review/sign-off is **maintainer + agents**, not a separate secops role. **Plan:** [Privacy, PHI/PII, and Leakage Hardening](plans/PRIVACY_PHI_PII_LEAKAGE_HARDENING_PLAN.md).
...
- [ ] **[P2]** **Watch Dependabot for a TruffleHog release that passes CI (no Sonar SHA false positive).** Dependabot PR #35 was superseded after TruffleHog’s SonarCloud detector falsely flagged Dependabot-style pinned Action SHAs (including the SonarSource scan action). **No active work until upstream fixes the detector** — do not suppress the detector or weaken secret-scan policy. When a new TruffleHog version is offered via Dependabot and the blocking secret scan passes on that PR, merge it; otherwise leave the pin as-is and recheck on the next Dependabot bump.
- [ ] **[P2]** **Watch for pylinac dropping the `pydicom<3` cap, then consider pydicom 3 + imagecodecs (BSD) decoder.** **BLOCKED UPSTREAM (2026-06-14; re-checked 2026-09-03):** the latest pylinac (**3.47.0** on PyPI — we pin **3.43.2**) *still* declares `pydicom<3,>=2.0`, so a pylinac bump alone does **not** unblock pydicom 3. pydicom 3 would unblock both the BSD `imagecodecs` decoder plugin (a fully-permissive alternative to the selected GDCM path) **and** highdicom. Action: periodically re-check pylinac's `requires_dist` for pydicom 3 support; when it lands, evaluate a coordinated pydicom 3 + pylinac bump (with ACR QA re-verification). **Spike done 2026-06-14 — pylinac 3.43.2 works under pydicom 3.0.2:** force-installed pydicom 3 past the cap; `import pylinac` + `ACRCT`/`ACRMRILarge` import fine, and offline **CatPhan504 (CT)** and **QuartDVT (MRI)** analyses give **identical results** on pydicom 2.4.5 and 3.0.2 (pydicom 3 still ships the `pixel_data_handlers`/`apply_rescale` shim). **The app's real ACR CT + ACR MRI Large modules also validated identical under pydicom 3** (full `raw_pylinac` metrics identical except timestamp). **Nuclear also validated: 8/8 types byte-identical** across pydicom 2.4.5 vs 3.0.2 (SimpleSensitivity multi-file not run). The `pydicom<3` cap is **conservative** — pylinac 3.43.2 runs fully on pydicom 3.0.2 with identical ACR + nuclear results. Remaining before actually bumping: a full [Dependency bump verification](plans/completed/DEPENDENCY_BUMP_VERIFICATION_PLAN.md) pass (real ACR CT/MRI data + pylinac test suite), and decide whether to override pylinac's metadata cap (safe for a controlled PyInstaller build) or wait for upstream. This makes the **pydicom 3 → imagecodecs (BSD) + highdicom** path viable sooner than expected. Only worth it if a fully permissive alternative becomes preferable — the selected GDCM path already solves the decoder blocker. (Separately, pylinac bumps up to 3.47.0 are available now, unrelated to this.) See [decoder strategy — options landscape](info/PYLIBJPEG_ALTERNATIVES_AND_DICOM_DECODER_STRATEGY.md#decoder-options-landscape--licenses-2026-06-14-spike-findings), [decoder spike plan](plans/supporting/DECODER_REPLACEMENT_SPIKE_PLAN.md), [HIGHDICOM_OVERVIEW §4a](info/HIGHDICOM_OVERVIEW.md), and the [Dependency bump verification plan](plans/completed/DEPENDENCY_BUMP_VERIFICATION_PLAN.md).
- [ ] **[P2]** **scikit-image deprecation in pylinac nuclear (FutureWarning).** pylinac 3.43.2 calls `remove_small_objects(min_size=)` / `remove_small_holes(area_threshold=)` (`nuclear.py:447-448`), deprecated in scikit-image 0.26.0 (we have no upper cap: `scikit-image>=0.22.0`). Will **break at skimage ≥2.0** and the replacement `max_size` changes boundary semantics (≤ vs <), which could shift nuclear results at size==2 when pylinac migrates. **Broader than nuclear:** ACR CT also emits `RegionProperties.filled_area` deprecation warnings (`pylinac/ct.py:418`, removed in skimage 2.0). Same root: scikit-image version drift vs pylinac 3.43.2. Action: consider pinning a scikit-image upper bound for stable behavior + silence, and re-verify nuclear results on any bump. Owner is upstream pylinac. See [PYLINAC_FLEXIBILITY_AND_WORKAROUNDS §7a](info/PYLINAC_FLEXIBILITY_AND_WORKAROUNDS.md). Confirmed `403 warnings` in full `pytest` 2026-09-03 (`6665 passed`) includes this plus the items below.
- [ ] **[P2]** **Refactor follow-up — split `volume_viewer_widget._build_controls`:** the 4-stream extraction work (tag-export controller, volume-viewer helpers, main-window status controller, MPR view-math) is done + merged per [Refactor extractions](plans/supporting/REFACTOR_TOP3_EXTRACTIONS_PLAN.md) (full suite green; post-merge smokes passed 2026-06). Remaining deferred follow-up: split `volume_viewer_widget._build_controls` (~535 Qt lines) into a `gui/volume/controls_builder.py` + VTK `pipeline.py` — needs interactive 3D verification.
- [ ] **[P2]** **Audit tracked files for unnecessary weight:** list tracked files by size/type, move build artifacts, large binaries, or regenerable outputs to gitignored paths or an LFS policy, and record decisions. Complements (does not replace) the PHI artifact gate.
...
- [ ] **[P1]** Further integrate pylinac and other automated QC analysis tools — **Partial:** Stage 1 spine plus the ACR CT/MRI full-metrics, batch, and MRI-SNR slice shipped; CatPhan/Nuclear/Stage 2+ remain open. The completed ACR slice is recorded in [Pylinac ACR full metrics export and MRI batch](plans/completed/PYLINAC_ACR_FULL_METRICS_EXPORT_AND_MRI_BATCH_PLAN.md). ([details](FUTURE_WORK_DETAIL_NOTES.md#integrating-pylinac-and-other-automated-qc-tools), [pylinac integration overview](info/PYLINAC_INTEGRATION_OVERVIEW.md), [additional automated QA analysis (ACR gaps + CT checks)](info/AUTOMATED_QA_ADDITIONAL_ANALYSIS.md), [Stage 1 implementation plan](plans/completed/PYLINAC_AND_AUTOMATED_QA_STAGE1_PLAN.md))
    ...
    - [ ] **[P2]** **ACR CT image thickness on export** — Pylinac 3.43.2 `ACRCT` has no measured slice-thickness module (ramp/FWHM). P0-GATE keeps that out of v1 Summary (and **no CT SNR**). Follow-up: harvest nominal DICOM **`SliceThickness`** (and `SpacingBetweenSlices` when present) onto the ACR CT run as a curated metric for CSV/XLSX. Do **not** treat the tag as an ACR Module 1 ramp measurement; that remains catalog **[C6](info/AUTOMATED_QA_ADDITIONAL_ANALYSIS.md)**.
    - [ ] **[P2]** **Consider ACR-compliant MRI PIU (percent image uniformity)** — Export today uses pylinac `uniformity_module.piu` (99th/1st percentile variant, not ACR’s ~1 cm² high/low regions on slice 7) and pylinac `piu_passed` at **85%/80%**, not ACR submission limits **≥ 87.5%** (<3 T) / **≥ 82%** (3 T). Consider a viewer-side ACR-procedure PIU as a **separate** curated metric from pylinac’s PIU; keep both labeled so they are not confused with **`mri_snr`**. Do not replace pylinac PIU in v1. **Reference:** [ACR phantom QA metrics and pylinac gaps](info/ACR_PHANTOM_QA_METRICS_AND_PYLINAC_GAPS.md) test 5.
...
- [ ] **[P1]** 3D visualization of DICOM datasets — **Partial:** VTK volume render shipped (`volume_renderer.py`, `VolumeRenderDialog`, toolbar **3D View**): presets (CT/MR + threshold), global opacity, W/L sync on preset change, modality-aware default preset, trackball navigation, background volume build. Open plan items below. **Plan:** [3D Volume Rendering](plans/3D_VOLUME_RENDERING_PLAN.md)
    ...
    - **Blank-frame GPU-fallback (implementation complete):** manual verification — [Manual Smoke Checks](#manual-smoke-checks). **Archived plan:** [Volume render fallback & memory hardening](plans/completed/VOLUME_RENDER_FALLBACK_AND_MEMORY_HARDENING_PLAN.md)
...
- [ ] **[P0]** **Commercial release readiness gate** — work the Tier 0 blockers (GPL decoder swap, project license, EULA/intended-use, tier model + enforcement, sales channel, code signing). **Gate:** [Commercial Release Readiness](COMMERCIAL_RELEASE_READINESS.md)
    - [ ] **[P0]** **Validate `python-gdcm` replacement for commercial release.** The GPL
      `pylibjpeg-libjpeg` runtime dependency has been removed locally, with the approved synthetic
      matrix and exact `.51` diagnostic contract implemented. Remaining: clean frozen/corpus
      validation on macOS, Windows, and Linux; native-asset notice/SBOM/vulnerability reconciliation;
      and release/compliance approval. **Active plan:** [GDCM decoder release validation](plans/supporting/GDCM_DECODER_PRODUCTIONIZATION_PLAN.md). **Completed record:** [local implementation](plans/completed/GDCM_DECODER_PRODUCTIONIZATION_LOCAL_IMPLEMENTATION_2026-07-29.md). **Background:** [decoder replacement spike](plans/supporting/DECODER_REPLACEMENT_SPIKE_PLAN.md).
- [ ] **[P2]** Review the IMAIOS (iOS) disclaimer as a model for our intended-use/medical disclaimer wording (feeds the EULA/intended-use work in the P0 commercial gate above).
