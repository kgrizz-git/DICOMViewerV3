...
- **Check installed handlers:** from the project venv, `PYTHONPATH=src python -c "from core.decoder_capabilities import available_decoder_backends; print(available_decoder_backends('1.2.840.10008.1.2.4.50'))"` should list `GDCM` when classic JPEG support is present (it prints `('GDCM', 'pylibjpeg', 'Pillow')` on a complete install). The `PYTHONPATH=src` prefix is required; without it the import fails with `ModuleNotFoundError: No module named 'core'`.
