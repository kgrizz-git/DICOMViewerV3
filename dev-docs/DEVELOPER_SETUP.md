...
The root [`README.md`](../README.md) launchers (`launch.bat`, `launch.command`)
create a virtual environment when needed, install requirements, and start the
...
- **Check installed handlers:** from the project venv, `PYTHONPATH=src python -c "from core.decoder_capabilities import available_decoder_backends; print(available_decoder_backends('1.2.840.10008.1.2.4.50'))"` should list `GDCM` when classic JPEG support is present (it prints `('GDCM', 'pylibjpeg', 'Pillow')` on a complete install). The `PYTHONPATH=src` prefix is required; without it the import fails with `ModuleNotFoundError: No module named 'core'`. That form is macOS/Linux; on Windows PowerShell use `$env:PYTHONPATH="src"; python -c "..."` and on `cmd.exe` `set PYTHONPATH=src && python -c "..."`.
