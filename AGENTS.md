...
- **Privacy / logging (local hooks):** Installed **`pre-commit`** / **`pre-push`** run **`scripts/git_hook_privacy_checks.py`** on staged **`src/*.py`** before branch-gated security scans (no **`traceback.print_exc()`**, heuristics on added lines). **`DICOMVIEWER_PRIVACY_HOOK=warn`** logs without blocking. Details: **`dev-docs/SECURITY_TOOLS_CLI_GUIDE.md`**, **`dev-docs/QUICK_REFERENCE_SECURITY.md`**.
