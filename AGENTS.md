...
- **Local Git hooks** (see `dev-docs/DEVELOPER_SETUP.md`): installed `pre-commit` prunes **`backups/`** on branch **`main`** (files **> 7 days** old by mtime) and **`WIP`** (**> 14 days**); then runs the **`main`** security gate as before.
