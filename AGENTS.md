...
- **Storage / billing**: Artifact and cache usage accrues in **GB-hours**; free plans include a small **artifact** allowance (see GitHub’s current billing docs). Large multi-OS **`upload-artifact`** outputs and long **`retention-days`** can exhaust quota quickly—see `dev-docs/info/GITHUB_ACTIONS_STORAGE_AND_BILLING.md`. The **Build Executables** workflow uploads **`dist/`** (and the Linux AppImage) only — **not** PyInstaller’s **`build/`** folder (debug analysis locally). Per-OS size notes and a maintainer baseline table: **`dev-docs/info/PYINSTALLER_BUNDLE_SIZE_AND_BASELINES.md`**. **`tests/test_pyinstaller_exclude_audit.py`** guards **`DICOMViewerV3.spec`** excludes (matplotlib backends / macOS-only PySide6 trims) against **`src/`** and **`tests/`** imports.
