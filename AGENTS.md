...
- **User-docs link check:** After editing files under `user-docs/` (or `dev-docs/README.md`), run `python scripts/check_user_docs_links.py` from the repo root, or `python -m pytest tests/test_user_docs_links.py -q` (same check; CI workflow **User docs links**).
