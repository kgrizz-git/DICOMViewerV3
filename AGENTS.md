...
- **basedpyright vs PySide6:** CI installs fresh deps, so it may resolve a newer PySide6 than your venv holds (the pin in `requirements.txt` is a `>=` floor). If local basedpyright suddenly reports many `QTreeWidgetItem | None` errors after `pip install -r requirements-dev.txt`, upgrade PySide6 to match CI and re-run `python scripts/check_basedpyright_errors.py` (tree code uses `gui.qt_tree_widget_utils.iter_tree_children`).
...
- **Git hooks and branch policy:** Never use `--no-verify`, disable or edit a hook to skip checks, or use an equivalent workaround for any commit, push, or verification gate unless the user gives explicit, specific approval to bypass that exact gate after its risk has been explained. A general request to commit, push, update a PR, or finish work is not approval. Never commit directly on `main` without **explicit maintainer authorization for that specific case**; only a direct maintainer instruction naming `main` qualifies — a general request to commit, push, update a PR, or finish work does not, and neither does passing CI, an approving review, or a prior "merge once checks pass" pattern. Use a named feature, plan, or hotfix branch otherwise, and do not merge without explicit maintainer instruction. This mirrors `dev-docs/CONTRIBUTING.md`. Git hooks (line complexity, PHI checks, ruff linting, etc.) are important quality gates. If hooks fail, fix the underlying issues or report the blocker rather than bypassing them.
