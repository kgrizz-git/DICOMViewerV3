...
- **Local Git hooks** (see `dev-docs/DEVELOPER_SETUP.md`): installed `pre-commit` prunes **`backups/`** on branches **`main`** and **`WIP`** (files **strictly older than 3 days** by mtime), stages removals with **`git add -u -- backups`**, then runs the **`main`** security gate as before.
