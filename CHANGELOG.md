...
- **Measurement line jump during handle drag**: While dragging a measurement handle, the group's position-sync logic could run after the handle's itemChange returned (when `_updating_handles` was already False), applying an incorrect delta and causing the line to jump. The group's `ItemPositionChange` and `ItemPositionHasChanged` handlers now also skip when `_handle_drag_in_progress` is True, so the line stays stable for the full duration of the drag.
