...
def _make(parent: QWidget | None = None) -> NoPixelPlaceholderOverlay:
    return NoPixelPlaceholderOverlay(parent)


class TestConfigure:
    def test_active_shows_bar(self, qapp):
        ov = _make()
    ...
    def test_active_button_without_callback_does_not_wire(self, qapp):
        ov = _make()
        ov.configure(active=True, show_open_button=True, open_callback=None)
        assert ov._btn.isVisible() is False
        assert ov._open_cb is None

    def test_button_hidden_when_requested_inactive(self, qapp):
        ov = _make()
...
class TestOpenCallback:
    def test_click_invokes_callback(self, qapp):
        cb = MagicMock()
        ov = _make()
        ov.configure(active=True, show_open_button=True, open_callback=cb)
        ov._on_open_clicked()
    ...
    def test_click_noop_without_callback(self, qapp):
        ov = _make()
        ov.configure(active=True, show_open_button=False, open_callback=None)
