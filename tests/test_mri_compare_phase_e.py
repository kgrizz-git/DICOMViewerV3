...
def test_build_mri_compare_json_document_schema_and_runs_length() -> None:
    ...
    fixed = datetime(2026, 4, 3, 12, 0, 0, tzinfo=UTC)
    ...
    batch.run_results[0].pdf_report_path = "/tmp/combined.pdf"  # NOSONAR - fake path on a test fixture, never touches the filesystem
    ...
    assert doc["combined_pdf_path"] == "/tmp/combined.pdf"  # NOSONAR - fake path in an assertion, never touches the filesystem
