...
class TestParserDuplicateTags(unittest.TestCase):
    def test_content_sequence_duplicate_code_value_keys(self):
        ...
        # Both Code Values survive, each keyed by the ContentSequence item that owns it.
        # They used to be flattened to root as "(0008, 0100)" and "(0008, 0100)#2",
        # which said nothing about which item each came from.
        code_keys = [k for k in tags if k.endswith("(0008, 0100)")]
        self.assertEqual(len(code_keys), 2, msg=f"keys sample: {list(tags)[:30]}")
        self.assertTrue(all("[" in k for k in code_keys), msg=f"expected path keys: {code_keys}")
