...
class _PaneTitleBarFrame(QFrame):
    ...
    def __init__(self, container: "SubWindowContainer", parent: QWidget | None = None) -> None:
        super().__init__(parent)
...
class SubWindowContainer(QFrame):
    ...
    def __init__(self, image_viewer: ImageViewer, parent=None):
        ...
        self.assigned_series_uid: str | None = None
        self.assigned_slice_index: int = 0

        # Border highlighting
        self.focus_border_width = 3
        self.normal_border_width = 1
        # Callable that returns the current accent preset ID (e.g. "blue").
        # Set by MultiWindowLayout after construction; falls back to default.
    ...
    def set_slice_sync_group_indicator(self, color: QColor | None) -> None:
        """
    ...
    def paintEvent(self, event) -> None:
        ...
        if self.is_focused and not self._suppress_focus_border_for_export:
            ...
            pen = QPen(self._focus_color(), self.focus_border_width)
            painter.setPen(pen)
    ...
    def set_assigned_series(self, series_uid: str | None, slice_index: int = 0) -> None:
        """
    ...
    def get_assigned_series(self) -> tuple[str | None, int]:
        """
