...
def open_wl_preset_manager(app: DICOMViewerApp) -> None:
    """Open Manage W/L Presets; persist user presets and rebuild active series list."""
    user_presets = app.config_manager.get_wl_user_presets()
    dlg = WLPresetManagerDialog(user_presets, parent=app.main_window)

    def _on_saved(new_presets: list[Any]) -> None:
        app.config_manager.set_wl_user_presets(new_presets)
        if hasattr(app, "slice_display_manager") and app.slice_display_manager is not None:
            app.slice_display_manager.rebuild_window_level_presets_for_current_series()

    dlg.presets_saved.connect(_on_saved)
    dlg.raise_()
    dlg.activateWindow()
    dlg.exec()
...
def open_quick_window_level(app: DICOMViewerApp) -> None:
    ...
    get_ctx: Callable[[], WLPresetMenuContext] | None = getattr(app.main_window, "_get_wl_preset_menu_context", None)
    get_legacy: Callable[[], list[Any]] | None = getattr(app.main_window, "_get_active_wl_presets", None)
    on_preset = (
        (lambda i: app.main_window._apply_wl_preset_requested.emit(i))
        if getattr(app.main_window, "_apply_wl_preset_requested", None) is not None
        else None
    )
    dlg = QuickWindowLevelDialog(
        ...
        unit=unit,
        on_preset_select=on_preset,
        get_wl_preset_menu_context=get_ctx if callable(get_ctx) else None,
        get_legacy_wl_presets=get_legacy if callable(get_legacy) else None,
        on_manage_wl_presets=app._open_wl_preset_manager,
    )
    dlg.raise_()
    dlg.activateWindow()
    dlg.exec()
...
def open_deep_anonymizer_export(app: DICOMViewerApp) -> None:
    """Handle File → Export with Deep Anonymization… request."""
    app.dialog_coordinator.open_deep_anonymizer_export()
...
def open_study_index_search(app: DICOMViewerApp) -> None:
    ...
    # Show the passphrase warning unless the user has permanently dismissed it.
    if not app.config_manager.get_study_index_passphrase_warning_dismissed():
        warn_dlg = StudyIndexPassphraseWarningDialog(parent=app.main_window)
        warn_dlg.exec()
        if warn_dlg.dismissed_permanently:
            app.config_manager.set_study_index_passphrase_warning_dismissed(True)
...
def open_structured_report_browser(
    app: DICOMViewerApp, subwindow_idx: int | None = None
) -> None:
    """
