...
def on_privacy_view_toggled(app: DICOMViewerApp, enabled: bool) -> None:
    ...
    if not enabled:
        app.main_window.show_toast_message(
            "Privacy mode is OFF — patient identifiers are now visible",
            timeout_ms=6000,
            severity="warning",
        )
    # Keep the shared privacy_action check state in sync regardless of which
    # trigger path (toolbar, menu, context menu, or keyboard shortcut) caused this.
    mw = app.main_window
    pa = getattr(mw, "privacy_action", None)
    if pa is not None and pa.isChecked() != enabled:
        pa.blockSignals(True)
        pa.setChecked(enabled)
        pa.blockSignals(False)
    if hasattr(mw, "_update_privacy_action"):
        mw._update_privacy_action()
...
def on_slice_slider_placement_changed(app: DICOMViewerApp, placement: str) -> None:
    """Handle the in-view slice/frame slider placement selection."""
    app.config_manager.set_slice_slider_placement(placement)
    placement = app.config_manager.get_slice_slider_placement()
    direction = app.config_manager.get_slice_slider_direction()
    set_slice_slider_options_all(app, placement, direction)
    app.main_window.set_slice_slider_placement_checked(placement)


def on_slice_slider_direction_changed(app: DICOMViewerApp, direction: str) -> None:
    """Handle the in-view slice/frame slider direction selection."""
    app.config_manager.set_slice_slider_direction(direction)
    placement = app.config_manager.get_slice_slider_placement()
    direction = app.config_manager.get_slice_slider_direction()
    set_slice_slider_options_all(app, placement, direction)
    app.main_window.set_slice_slider_direction_checked(direction)
