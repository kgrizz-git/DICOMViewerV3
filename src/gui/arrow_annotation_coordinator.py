...
class ArrowAnnotationCoordinator:
    ...
    def __init__(
        ...
        self._arrow_move_tracking: dict[object, dict[str, Any]] = {}  # Track arrow moves for batching
        self._move_batch_timer: QTimer | None = None
        ...
        if key in self.arrow_annotation_tool.arrows:
            for arrow_item in self.arrow_annotation_tool.arrows[key]:
                ...
                if arrow_item not in self._arrow_move_tracking:
                    ...
                    annotation_debug(" ArrowAnnotationCoordinator.display_arrows_for_slice: stored initial position for existing arrow")

        # Update line ends for current zoom so no gap between line and arrowhead
        self._update_arrow_lines_for_view_scale()
    ...
    def _on_arrow_moved(self, arrow_item) -> None:
        ...
        try:
            ...
            if getattr(arrow_item, '_updating_position', False):
                annotation_debug(" ArrowAnnotationCoordinator._on_arrow_moved: skipping (programmatic update), _updating_position=True")
                return
            ...
            if not is_tracked:
                ...
                if pre_drag_start is not None and pre_drag_end is not None:
                    ...
                    annotation_debug(" ArrowAnnotationCoordinator._on_arrow_moved: First move, using pre_drag positions as initial")
                    ...
                    annotation_debug(" ArrowAnnotationCoordinator._on_arrow_moved: First move, using pre_move positions as initial")
                else:
                    ...
                    annotation_debug(" ArrowAnnotationCoordinator._on_arrow_moved: First move, no pre_drag/pre_move, using current")
                ...
                annotation_debug(" ArrowAnnotationCoordinator._on_arrow_moved: Continuing tracking, updated current positions")
    ...
    def _finalize_arrow_move(self, arrow_item) -> None:
        ...
        if arrow_item not in self._arrow_move_tracking:
            annotation_debug(" ArrowAnnotationCoordinator._finalize_arrow_move: arrow not in tracking, returning")
            return

        # Verify arrow is still valid
        if arrow_item is None or not hasattr(arrow_item, 'start_point'):
            annotation_debug(" ArrowAnnotationCoordinator._finalize_arrow_move: arrow invalid, removing from tracking")
        ...
        if position_changed and has_manager and has_scene:
            ...
            if urm is None or scene is None:
                annotation_debug(" ArrowAnnotationCoordinator._finalize_arrow_move: skip command (urm or scene became None)")
            else:
                from utils.undo_redo import ArrowAnnotationMoveCommand
                annotation_debug(" ArrowAnnotationCoordinator._finalize_arrow_move: creating ArrowAnnotationMoveCommand")
                ...
                annotation_debug(f" ArrowAnnotationCoordinator._finalize_arrow_move: executing command, undo stack size before={len(urm.undo_stack)}")
                urm.execute_command(command)
                annotation_debug(f" ArrowAnnotationCoordinator._finalize_arrow_move: command executed, undo stack size after={len(urm.undo_stack)}")
            # Update undo/redo state after command execution
            if self.update_undo_redo_state_callback:
                self.update_undo_redo_state_callback()
        else:
        ...
        if hasattr(arrow_item, '_pre_drag_end_point'):
            ...
            annotation_debug(" ArrowAnnotationCoordinator._finalize_arrow_move: removed from tracking")
