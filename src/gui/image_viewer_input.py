...
def _make_magnifier_cursor() -> QCursor:
    """
    Return a magnifier QCursor (32×32 logical, hotspot top-left of lens).
    Loads resources/cursors/magnifier.png when available; falls back to
    a QPainter-drawn version so the cursor always works without the asset.
    """
    _asset = os.path.join(
        os.path.dirname(__file__), "..", "..", "resources", "cursors", "magnifier.png"
    )
    _asset = os.path.normpath(_asset)
    if os.path.isfile(_asset):
        px = QPixmap(_asset)
        if not px.isNull():
            # Scale to 32×32 logical if the file is HiDPI (64×64)
            if px.width() > 32:
                px = px.scaled(32, 32, Qt.AspectRatioMode.KeepAspectRatio,
                               Qt.TransformationMode.SmoothTransformation)
            return QCursor(px, 8, 8)  # hotspot: top-left of lens circle
    ...
    hx2 = hx1 + 10 * math.cos(angle)
    hy2 = hy1 + 10 * math.sin(angle)

    # Dark outer outline (handle then lens)
    ...
    # Light grey handle core
    p.setPen(QPen(QColor(140, 140, 140, 255), 3))
    p.drawLine(int(hx1), int(hy1), int(hx2), int(hy2))

    # Semi-transparent lens fill + ring
    p.setBrush(QBrush(QColor(160, 160, 160, 90)))
    p.setPen(QPen(QColor(30, 30, 30, 255), 1))
    p.drawEllipse(cx - r, cy - r, r * 2, r * 2)
    p.end()

    return QCursor(px, 8, 8)

from utils.debug_flags import DEBUG_MAGNIFIER, DEBUG_NAV
...
class ImageViewerInputMixin:
    ...
    def wheelEvent(self, event: QWheelEvent) -> None:
        ...
        Ctrl/Cmd+wheel adjusts the shared corner-overlay font size.
        """
        # Keep this before the normal wheel-mode routing so the modifier has the
        # same meaning whether wheel mode is Slice or Zoom. Native pinch gestures
        # remain available for image zooming (handled in ``event`` above).
        if event.modifiers() & (
    ...
    def set_mouse_mode(self, mode: str) -> None:
        ...
        if mode == "select":
            ...
            self.roi_drawing_mode = None
            self.setDragMode(QGraphicsView.DragMode.NoDrag)
            self.setCursor(_make_magnifier_cursor())
    ...
    def _apply_cursor_for_mouse_mode(self) -> None:
        ...
        if mode == "select":
            self.setCursor(Qt.CursorShape.ArrowCursor)
        elif mode in ("roi_ellipse", "roi_rectangle", "measure", "measure_angle", "crosshair", "arrow_annotation") or mode == "auto_window_level":
            self.setCursor(Qt.CursorShape.CrossCursor)
        elif mode in ("zoom", "magnifier"):
            self.setCursor(_make_magnifier_cursor())
        elif mode == "text_annotation":
    ...
    def set_roi_drawing_mode(self, mode: str | None) -> None:
        """
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        Args:
            event: Mouse event
        """
        # Immediately hide the frame slider overlay when the user clicks outside it.
        _slider_overlay = getattr(self, "_slider_overlay", None)
        if (
            _slider_overlay is not None
            and _slider_overlay.isVisible()
            and not _slider_overlay.is_interacting()
            and not _slider_overlay.geometry().contains(event.position().toPoint())
        ):
            _slider_overlay.hide_immediately()
        ...
        if (
            ...
            and not bool(parent.is_focused)
    ...
    def drawForeground(self, painter, rect) -> None:
        """Draw drag-and-drop target indicator over the scene foreground."""
        super().drawForeground(painter, rect)
        if not getattr(self, "_drag_active", False):
            return
        # Reset to viewport (device) coordinates so the border is always
        # a fixed-pixel inset regardless of zoom/pan transform.
        painter.save()
        painter.resetTransform()
        accent = self.palette().highlight().color()
        accent.setAlpha(200)
        pen = QPen(accent, 3, Qt.PenStyle.DashLine)
        pen.setDashPattern([6.0, 4.0])
        painter.setPen(pen)
        painter.setBrush(Qt.BrushStyle.NoBrush)
        margin = 4
        r = self.viewport().rect().adjusted(margin, margin, -margin, -margin)
        painter.drawRect(r)
        painter.restore()
    ...
    def _update_slider_visibility_from_mouse(self, pos) -> None:
        ...
        how close the mouse pointer is to the configured viewport edge.

        Args:
            pos: QPoint in viewport coordinates (from event.position().toPoint()).
        """
        ACTIVATION_ZONE_PX = 40
        ...
        placement = getattr(self, "_slice_slider_placement", "bottom")
        if placement == "top":
            distance_to_edge = pos.y()
        elif placement == "left":
            distance_to_edge = pos.x()
        elif placement == "right":
            distance_to_edge = viewport.width() - pos.x()
        else:
            distance_to_edge = viewport.height() - pos.y()
        ...
        if distance_to_edge <= ACTIVATION_ZONE_PX:
            overlay.reveal()
