...
def _get_resource_path(relative_path: str) -> str:
    ...
    if getattr(sys, 'frozen', False):
        ...
        base_path = Path(cast(str, getattr(sys, "_MEIPASS")))  # noqa: B009 - PyInstaller sets this only in frozen builds
    else:
...
class MainWindow(QMainWindow):
    ...
    deep_anonymizer_export_requested = Signal()  # File → Export with Deep Anonymization…
    ...
    keyboard_shortcuts_requested = Signal()  # Emitted when Keyboard Shortcuts dialog is requested
    _apply_wl_preset_requested = Signal(int)  # Emitted when a W/L preset is selected from toolbar dropdown
    ...
    slice_slider_placement_changed = Signal(str)  # "bottom", "top", "left", or "right"
    slice_slider_direction_changed = Signal(str)  # "first_at_start" or "first_at_end"
    ...
    toggle_overlay_requested = Signal()           # Toolbar overlay-toggle button
    cine_play_pause_requested = Signal()          # Toolbar cine play/pause button
    ...
    smooth_when_zoomed_action: QAction | None = None
    scale_markers_action: QAction | None = None
    direction_labels_action: QAction | None = None
    slice_slider_action: QAction | None = None
    slice_slider_menu: QMenu | None = None
    slice_slider_placement_menu: QMenu | None = None
    slice_slider_direction_menu: QMenu | None = None
    slice_slider_placement_actions: ClassVar[dict[str, QAction]] = {}
    slice_slider_direction_actions: ClassVar[dict[str, QAction]] = {}
    show_instances_separately_action: QAction | None = None
    show_left_pane_action: QAction | None = None
    show_right_pane_action: QAction | None = None
    show_series_navigator_action: QAction | None = None
    show_navigator_slice_frame_count_action: QAction | None = None
    fullscreen_action: QAction | None = None
    show_window_slot_map_action: QAction | None = None
    slice_sync_action: QAction | None = None
    slice_location_lines_enable_action: QAction | None = None
    slice_location_lines_same_group_only_action: QAction | None = None
    slice_location_lines_focused_only_action: QAction | None = None
    slice_location_lines_show_slab_bounds_action: QAction | None = None
    copy_annotation_action: QAction | None = None
    cut_annotation_action: QAction | None = None
    paste_annotation_action: QAction | None = None
    undo_tag_edit_action: QAction | None = None
    redo_tag_edit_action: QAction | None = None
    layout_1x1_action: QAction | None = None
    layout_1x2_action: QAction | None = None
    layout_2x1_action: QAction | None = None
    layout_2x2_action: QAction | None = None
    # Three-pane asymmetric layout actions (LP-A); set by the menu builder.
    layout_1p2r_action: QAction | None = None
    layout_2l1_action: QAction | None = None
    layout_2t1_action: QAction | None = None
    layout_1p2b_action: QAction | None = None
    # Back-reference + W/L-preset menu callbacks wired by ``DICOMViewerApp``.
    _app_ref: Any = None
    _get_wl_preset_menu_context: Any = None
    _open_wl_preset_manager: Any = None

    # Populated by ``build_main_toolbar`` (Optional until toolbar is built).
    main_toolbar: QToolBar | None = None
    scroll_wheel_mode_combo: QComboBox | None = None
    mouse_mode_ellipse_roi_action: QAction | None = None
    mouse_mode_rectangle_roi_action: QAction | None = None
    mouse_mode_measure_action: QAction | None = None
    mouse_mode_measure_angle_action: QAction | None = None
    mouse_mode_text_annotation_action: QAction | None = None
    mouse_mode_arrow_annotation_action: QAction | None = None
    mouse_mode_crosshair_action: QAction | None = None
    mouse_mode_zoom_action: QAction | None = None
    mouse_mode_magnifier_action: QAction | None = None
    mouse_mode_pan_action: QAction | None = None
    ...
    next_series_action: QAction | None = None
    view_3d_action: QAction | None = None
    create_3d_action: QAction | None = None

    # Populated when series navigator bar is assembled (may stay None until then).
    window_slot_map_widget: WindowSlotMapWidget | None = None
    series_navigator_container: QWidget | None = None

    # Populated by ``build_main_toolbar`` (split-button and W/L widgets).
    _open_split_btn: QToolButton | None = None
    _wl_toolbar_btn: QToolButton | None = None
    # Icon registries (list of (target, icon_name) for theme refresh).
    _toolbar_icon_registry: ClassVar[list[Any]] = []
    _menu_icon_registry: ClassVar[list[Any]] = []
    # Callable slots stamped on by the toolbar builder.
    refresh_toolbar_icon_theme: Any | None = None
    apply_toolbar_label_style: Any | None = None
    ...
    def __init__(self, config_manager: ConfigManager | None = None):
        ...
        # Privacy action (shared by View menu and toolbar — single source of truth).
        # checked=True means privacy ON (identifiers masked).
        self.privacy_action = QAction("&Privacy View", self)
        self.privacy_action.setCheckable(True)
        self.privacy_action.setChecked(self.config_manager.get_privacy_view())
        self.privacy_action.setShortcut(QKeySequence("Ctrl+P"))
        self.privacy_action.triggered.connect(self._on_privacy_toggled)
    ...
    def _create_status_bar(self) -> None:
        ...
        self._status_controller = MainWindowStatusController(self.statusBar())
        # Keep label references on the window for backward compatibility (e.g. the
        # external pixel-info update in core/view_state_handlers.py).
        self.file_study_label = self._status_controller.file_study_label
        self.zoom_preset_label = self._status_controller.zoom_preset_label
        self.pixel_info_label = self._status_controller.pixel_info_label
        ...
        *,
        ...
        Args:
            ...
            position: ``top-center`` (default), ``bottom-center``, or ``center`` of
                the main window client area (widget coordinates).
    ...
    def _apply_theme(self) -> None:
        ...
        accent_id = self.config_manager.get_accent()
        stylesheet = get_theme_stylesheet(
            theme, white_checkmark_path, black_checkmark_path, accent_id
        )
        ...
        # Refresh toolbar icons so stroke colour matches the new theme.
        if self.refresh_toolbar_icon_theme is not None:
            from gui.main_window_toolbar_builder import _icon_color
            self.refresh_toolbar_icon_theme(_icon_color(theme))
...
<body>
    ...
    def set_rescale_toggle_state(self, checked: bool) -> None:
        """
        Sync the 'Use rescaled values' checkbox in the right panel to the given state.

        Args:
            checked: True if using rescaled values, False if using raw values
        """
        if self.use_rescaled_values_checkbox is not None:
            self.use_rescaled_values_checkbox.blockSignals(True)
            self.use_rescaled_values_checkbox.setChecked(checked)
            self.use_rescaled_values_checkbox.blockSignals(False)
    ...
    def update_status(self, message: str) -> None:
        ...
        """
        self._status_controller.set_file_study(message)

    def update_zoom_preset_status(
        self,
        zoom: float,
    ...
    def toggle_navigator_slice_frame_count_badge(self, checked: bool) -> None:
        ...
        if nav is not None:
            _badge_fn = getattr(nav, "set_show_slice_frame_count_badge", None)
            if callable(_badge_fn):
                _badge_fn(bool(checked))
    ...
    def set_window_slot_map_callbacks(
        ...
        if widget is not None:
            widget.set_callbacks(
                ...
                get_accent_id=self.config_manager.get_accent,
            )
    ...
    def _on_layout_changed(self, layout_mode: str) -> None:
        ...
        Args:
            layout_mode: Layout mode (1x1, 1x2, 2x1, 2x2, 1+2R, 2L+1, 2T+1, 1+2B)
        """
        if DEBUG_LAYOUT:
            import traceback
        ...
        if getattr(self, "layout_1p2r_action", None) is not None:
            self.layout_1p2r_action.setChecked(layout_mode == "1+2R")
        if getattr(self, "layout_2l1_action", None) is not None:
            self.layout_2l1_action.setChecked(layout_mode == "2L+1")
        if getattr(self, "layout_2t1_action", None) is not None:
            self.layout_2t1_action.setChecked(layout_mode == "2T+1")
        if getattr(self, "layout_1p2b_action", None) is not None:
            self.layout_1p2b_action.setChecked(layout_mode == "1+2B")
