...
class SeriesNavigator(QWidget):
    ...
    def __init__(self, dicom_processor: DICOMProcessor, parent=None):
        ...
        # Deferred thumbnail generation queue — filled during update_series_list,
        # drained one-at-a-time via QTimer.singleShot(0, ...) so the UI stays responsive.
        self._pending_thumbnail_jobs: list[tuple[str, str, Any, list[Any] | None]] = []
        self._pending_instance_thumbnail_jobs: list[tuple[str, str, int, Any, list[Any] | None]] = []
        ...
        self._mpr_section_container: QWidget | None = None
    ...
    def build_instance_entries(self, datasets: list[Dataset]) -> list[tuple[int, Dataset, str]]:
        """Return navigator instance entries for a flattened series."""
        return build_instance_entries_for_navigator(datasets)
    ...
    def update_series_list(self, studies: dict[str, Any], current_study_uid: str, current_series_uid: str) -> None:
        ...
        """
        series_count = sum(len(study_series) for study_series in studies.values())
        perf_mark(
            "first_paint.navigator.update_series_list.start",
            studies=len(studies),
            series=series_count,
        )
    ...
    def _process_next_thumbnail(self) -> None:
        """Generate one pending series thumbnail, update the widget, schedule the next."""
        if self._pending_thumbnail_jobs:
            study_uid, series_uid, first_dataset, series_datasets = (
                self._pending_thumbnail_jobs.pop(0)
            )
            with perf_timer("first_paint.navigator.generate_series_thumbnail"):
                thumbnail_image = self._generate_thumbnail(first_dataset, series_datasets)
            cache_key = (study_uid, series_uid)
            if thumbnail_image:
                self.thumbnail_cache[cache_key] = thumbnail_image
            composite_key = f"{study_uid}:{series_uid}"
            thumb_widget = self.thumbnails.get(composite_key)
            if thumb_widget is not None:
                with perf_timer("first_paint.navigator.apply_series_thumbnail"):
                    thumb_widget.update_thumbnail_image(thumbnail_image)

        # After series thumbnails, process instance thumbnails
        if self._pending_thumbnail_jobs:
            QTimer.singleShot(0, self._process_next_thumbnail)
        elif self._pending_instance_thumbnail_jobs:
            QTimer.singleShot(0, self._process_next_instance_thumbnail)

    def _process_next_instance_thumbnail(self) -> None:
        """Generate one pending instance thumbnail, update the widget, schedule the next."""
        if not self._pending_instance_thumbnail_jobs:
            return
        study_uid, series_uid, slice_index, instance_dataset, series_datasets = (
            self._pending_instance_thumbnail_jobs.pop(0)
        )
        with perf_timer("first_paint.navigator.generate_instance_thumbnail"):
            thumbnail_image = self._generate_thumbnail(instance_dataset, series_datasets)
        cache_key = (study_uid, series_uid, slice_index)
        if thumbnail_image:
            self.instance_thumbnail_cache[cache_key] = thumbnail_image
        instance_composite_key = f"{study_uid}:{series_uid}:{slice_index}"
        thumb_widget = self.instance_thumbnails.get(instance_composite_key)
        if thumb_widget is not None:
            with perf_timer("first_paint.navigator.apply_instance_thumbnail"):
                thumb_widget.update_thumbnail_image(thumbnail_image)

        if self._pending_instance_thumbnail_jobs:
            QTimer.singleShot(0, self._process_next_instance_thumbnail)
    ...
    def _get_instance_thumbnail_key(self, study_uid: str, series_uid: str, slice_index: int) -> str | None:
    ...
    def _resolve_thumbnail_window_level(
        self,
        dataset: Dataset,
        series_datasets: list[Dataset] | None,
    ...
    def _generate_thumbnail(
        self,
        dataset: Dataset,
        series_datasets: list[Dataset] | None = None,
    ) -> Image.Image | None:
        """
    ...
    def set_current_series(self, series_uid: str, study_uid: str | None = None) -> None:
        """
