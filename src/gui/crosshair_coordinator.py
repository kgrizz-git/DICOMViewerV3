...
class CrosshairCoordinator:
    ...
    def __init__(
        ...
        get_use_rescaled_values: Callable[[], bool] | None = None
    ):
        """
    ...
    def handle_crosshair_clicked(self, pos: QPointF, pixel_value_str: str, x: int, y: int, z: int) -> None:
        ...
        # Set current slice context (always define UIDs for undo command below)
        study_uid = ""
        series_uid = ""
        ...
        if current_dataset is not None:
            ...
            # Calculate patient coordinates. current_dataset is the clicked
            # slice, so its ImagePositionPatient already encodes the slice z;
            # slice_index must be 0 to avoid double-counting the out-of-plane offset.
            patient_coords = pixel_to_patient_coordinates(current_dataset, x, y, 0)
            if patient_coords:
    ...
    def _update_crosshair_pixel_values(self, crosshair_item, new_pos: QPointF) -> None:
        ...
        try:
            ...
            # slice_index=0: current_dataset's IPP already encodes this slice's z.
            patient_coords = pixel_to_patient_coordinates(current_dataset, x, y, 0)
            if patient_coords:
                px, py, pz = patient_coords
                pixel_value_str = f"{pixel_value_str}\nPatient: ({px:.2f}, {py:.2f}, {pz:.2f}) mm"
