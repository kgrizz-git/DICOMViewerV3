...
# Map the selected nuclear class -> (analysis_type, JSON/CSV filename stem).
_NUCLEAR_CLASS_ROUTING = {
    PLANAR_UNIFORMITY_CLASS: (NUCLEAR_PLANAR_UNIFORMITY, "qa-nuclear-planar-uniformity"),
    FOUR_BAR_RESOLUTION_CLASS: (
        NUCLEAR_FOUR_BAR_RESOLUTION,
        "qa-nuclear-four-bar-resolution",
    ),
    QUADRANT_RESOLUTION_CLASS: (
        NUCLEAR_QUADRANT_RESOLUTION,
        "qa-nuclear-quadrant-resolution",
    ),
    CENTER_OF_ROTATION_CLASS: (
        NUCLEAR_CENTER_OF_ROTATION,
        "qa-nuclear-center-of-rotation",
    ),
    TOMOGRAPHIC_RESOLUTION_CLASS: (
        NUCLEAR_TOMOGRAPHIC_RESOLUTION,
        "qa-nuclear-tomographic-resolution",
    ),
    MAX_COUNT_RATE_CLASS: (NUCLEAR_MAX_COUNT_RATE, "qa-nuclear-max-count-rate"),
    TOMOGRAPHIC_UNIFORMITY_CLASS: (
        NUCLEAR_TOMOGRAPHIC_UNIFORMITY,
        "qa-nuclear-tomographic-uniformity",
    ),
    TOMOGRAPHIC_CONTRAST_CLASS: (
        NUCLEAR_TOMOGRAPHIC_CONTRAST,
        "qa-nuclear-tomographic-contrast",
    ),
    SIMPLE_SENSITIVITY_CLASS: (
        NUCLEAR_SIMPLE_SENSITIVITY,
...
class QAAppFacade:
    ...
    def show_qa_result_dialog(self, title: str, result: QAResult) -> None:
        ...
        Nuclear runs use a dedicated per-frame table dialog; ACR runs use the
        shared status message box (which includes PDF / stock-pylinac lines that
        do not apply to nuclear).
        """
        app = self._app
        ...
        if profile.get("module") == "pylinac.nuclear" or str(
            ...
            show_nuclear_result_dialog(app.main_window, title, result)
            return
    ...
    def export_qa_results(
        ...
        if not path.lower().endswith(".json"):
            path = f"{path}.json"
        payload = build_single_run_document(
            result, app_version=APP_VERSION, inputs=inputs
        )
        with open(path, "w", encoding="utf-8") as handle:
            json.dump(payload, handle, indent=2)
        app.main_window.update_status(f"Saved QA JSON: {path}")
    ...
    def offer_extent_retry(
        ...
        *,
        ...
        mb.setText(
            ...
            "Retry with a small tolerance? This uses viewer integration mode and is recorded "
            "in the JSON export."
        )
    ...
    def open_nuclear_qc_analysis(self) -> None:
        """Open the nuclear-medicine QC (pylinac.nuclear) analysis flow.
        ...
        if use_focused:
            choice = QMessageBox(app.main_window)
            choice.setWindowTitle("Nuclear QC Source")
            choice.setText("Use the focused image or choose a DICOM file?")
            choice.addButton("Use Focused Image", QMessageBox.ButtonRole.AcceptRole)
            choice.addButton("Choose File", QMessageBox.ButtonRole.ActionRole)
            choice.addButton(QMessageBox.StandardButton.Cancel)
            choice.setWindowFlags(choice.windowFlags() | Qt.WindowType.WindowStaysOnTopHint)
            choice.activateWindow()
            choice.raise_()
            ...
            if clicked is None or clicked == choice.button(QMessageBox.StandardButton.Cancel):
                return
            if clicked.text() == "Use Focused Image":
                input_path = ordered_paths[0]
            else:
                use_focused = False

        if not use_focused:
            picked = app.file_dialog.open_files(app.main_window)
            if not picked:
                return
            input_path = picked[0]
            # Focused-series identity does not apply to an arbitrary picked file;
            # read identity/modality best-effort so the NM preflight has teeth.
            assert input_path is not None
            study_uid, series_uid, modality = self._read_identity(input_path)

        if not input_path:
            return

        preflight: list[str] = []
        modality_warn = modality_preflight_warning(modality, "NM")
        if modality_warn:
            preflight.append(modality_warn)
        if not self.user_confirms_preflight(preflight):
            return

        options = prompt_nuclear_options(app.main_window)
        if options is None:
            return

        analysis_type, default_stem = _NUCLEAR_CLASS_ROUTING.get(
            options.analysis_class,
            (NUCLEAR_PLANAR_UNIFORMITY, "qa-nuclear"),
        )
        request = QARequest(
            analysis_type=analysis_type,
            dicom_paths=[input_path],
            study_uid=study_uid,
            series_uid=series_uid,
            modality=modality,
            preflight_warnings=preflight,
            nuclear_options=options,
        )
        json_inputs: dict[str, Any] = {
            "input_path": input_path,
            "nuclear_analysis_class": options.analysis_class,
            "options": options.analyze_kwargs(),
            "preflight_warnings": preflight,
        }
        self.start_qa_worker(
            request,
            progress_title="Nuclear Medicine QC",
            progress_label="Running nuclear QC analysis...",
            result_dialog_title="Nuclear Medicine QC",
            json_default_stem=default_stem,
            json_inputs=json_inputs,
        )

    @staticmethod
    def _read_identity(path: str) -> tuple[str, str, str]:
        """Best-effort (study_uid, series_uid, modality) from a DICOM file header."""
        try:
            import pydicom
