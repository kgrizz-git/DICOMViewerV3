...
class ImageViewer(ImageViewerInputMixin, ImageViewerViewMixin, QGraphicsView):
    ...
    create_3d_view_requested = Signal()  # Emitted when "3D Volume Render…" is chosen from context menu
    ...
    def __init__(self, parent: QWidget | None = None, config_manager=None):
        """
        ...
        self.inversion_state_changed_callback: Callable[[bool], None] | None = None
        ...
        self.orientation_changed_callback: Callable[[bool, bool, int], None] | None = None

        # Callbacks to get current dataset and slice index for pixel value display
        self.get_current_dataset_callback: Callable[[], Any] | None = None
        self.get_current_slice_index_callback: Callable[[], int] | None = None
        self.get_use_rescaled_values_callback: Callable[[], bool] | None = None
        # Callback to get file path for current slice (for "Show file" context menu)
        self.get_file_path_callback: Callable[[], str | None] | None = None
        ...
        self.roi_drawing_mode: str | None = None  # "rectangle", "ellipse", or None
        self.roi_drawing_start: QPointF | None = None

        # Measurement state
        self.measuring = False
        self.measurement_start_pos: QPointF | None = None

        # Text annotation state
        self.text_annotating = False
        self.text_annotation_start_pos: QPointF | None = None

        # Arrow annotation state
        self.arrow_annotating = False
        self.arrow_annotation_start_pos: QPointF | None = None

        # Zoom mode state
        self.zoom_start_pos: QPointF | None = None
        self.zoom_start_zoom: float | None = None
        ...
        self.magnifier_widget: MagnifierWidget | None = None
        ...
        self.handle_drag_magnifier_widget: MagnifierWidget | None = None
        ...
        self.get_cine_loop_state_callback: Callable[[], bool] | None = None
        # Whether cine is currently playing (for context menu play/pause label)
        self.get_cine_is_playing_callback: Callable[[], bool] | None = None
        # Whether this pane has content that Clear This Window can remove
        self.get_clear_this_window_enabled_callback: Callable[[], bool] | None = None
        self.get_3d_volume_render_enabled_callback: Callable[[], bool] | None = None
        self.get_available_series_callback: Callable[[], list[tuple[str, str]]] | None = None

        # Slice-location line visibility and mode (optional; filled by app for context menu sync)
        self.get_slice_location_lines_visible_callback: Callable[[], bool] | None = None
        self.get_slice_location_lines_same_group_only_callback: Callable[[], bool] | None = None
        self.get_slice_location_lines_focused_only_callback: Callable[[], bool] | None = None
        self.get_slice_location_lines_mode_callback: Callable[[], str] | None = None
        ...
        self._pan_update_timer: QTimer | None = None

        # Right mouse drag for window/level adjustment
        self.right_mouse_drag_start_pos: QPointF | None = None
        self.right_mouse_drag_start_center: float | None = None
        self.right_mouse_drag_start_width: float | None = None
        ...
        self.get_window_level_presets_callback: Callable[[], list[tuple[float, float, bool, str | None]]] | None = None
        self.get_current_preset_index_callback: Callable[[], int] | None = None
        self.get_wl_preset_menu_context_callback: Callable[[], Any] | None = None
        self.manage_wl_presets_callback: Callable[[], None] | None = None

        # Callback to get ROI from item (set from main.py)
        self.get_roi_from_item_callback: Callable[[object], ROIItem | None] | None = None
        self.delete_all_rois_callback: Callable[[], None] | None = None

        # Callbacks for projection state (set from main.py)
        self.get_projection_enabled_callback: Callable[[], bool] | None = None
        self.get_projection_type_callback: Callable[[], str] | None = None
        self.get_projection_slice_count_callback: Callable[[], int] | None = None
        ...
        self._slice_slider_placement: str = DEFAULT_SLIDER_PLACEMENT
        self._slice_slider_direction: str = DEFAULT_SLIDER_DIRECTION
        ...
        self.slider_navigate_callback: Callable[[int], None] | None = None
    ...
    def set_slice_slider_options(self, placement: str, direction: str) -> None:
        """Apply in-window slider placement and first-slice/frame direction."""
        self._slice_slider_placement = normalize_slider_placement(placement)
        self._slice_slider_direction = normalize_slider_direction(direction)
        self._slider_overlay.configure(
            self._slice_slider_placement,
            self._slice_slider_direction,
        )
    ...
    def _on_slider_overlay_value_changed(self, value: int) -> None:
        ...
        if self.slider_navigate_callback is not None:
            self.slider_navigate_callback(
                slider_value_to_logical_index(
                    value,
                    minimum=self._slider_overlay.minimum(),
                    maximum=self._slider_overlay.maximum(),
                )
            )

    @staticmethod
    def compute_slider_overlay_geometry(
        viewport_width: int,
        viewport_height: int,
        placement: str,
    ) -> QRect | None:
        """Return centered edge geometry for the slider overlay."""
        placement = normalize_slider_placement(placement)
        margin = 8
        ratio = 0.50
        min_long = 180
        horizontal_height = 44
        vertical_width = 88

        if placement in ("bottom", "top"):
            if viewport_width < 100 or viewport_height < horizontal_height:
                return None
            available = max(0, viewport_width - (2 * margin))
            if available <= 0:
                return None
            width = int(round(viewport_width * ratio))
            width = min(width, available)
            if available >= min_long:
                width = max(width, min_long)
            x = max(0, (viewport_width - width) // 2)
            y = 0 if placement == "top" else viewport_height - horizontal_height
            return QRect(x, y, width, horizontal_height)

        if viewport_width < vertical_width or viewport_height < 100:
            return None
        available = max(0, viewport_height - (2 * margin))
        if available <= 0:
            return None
        height = int(round(viewport_height * ratio))
        height = min(height, available)
        if available >= min_long:
            height = max(height, min_long)
        x = 0 if placement == "left" else viewport_width - vertical_width
        y = max(0, (viewport_height - height) // 2)
        return QRect(x, y, vertical_width, height)

    def _reposition_slider_overlay(self) -> None:
        """
        Re-geometry the slider overlay to sit on the configured viewport edge.
        Called from resizeEvent and after layout changes.
        """
        vp = self.viewport()
        if vp is None:
            return
        geometry = self.compute_slider_overlay_geometry(
            vp.width(),
            vp.height(),
            self._slice_slider_placement,
        )
        if geometry is None:
            self._slider_overlay.setVisible(False)
            return
        self._slider_overlay.setGeometry(geometry)
