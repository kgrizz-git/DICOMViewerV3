...
def _pen_for_line(color: tuple[int, int, int], width: int, dash: bool = False) -> QPen:
    """
    ...
    When dash=True a DashLine style is used (MPR reference lines).
    """
...
class SliceLocationLineManager:
    ...
    def set_scene(self, scene: QGraphicsScene | None) -> None:
    ...
    def update_lines(
        ...
        for seg in segments:
            ...
            is_mpr_ref = line_id.startswith("mpr_ref:")
            ...
            if item is not None:
                ...
                item.setPen(_pen_for_line(color, lw, dash=is_mpr_ref))
