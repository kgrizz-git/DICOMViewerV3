...
from core.dicom_rescale import get_rescale_parameters as _get_rescale_parameters
from core.direction_labels import (
    apply_orientation_to_labels,
...
class ImageViewerViewMixin:
    ...
    def _extract_pixel_spacing(self, dataset: Any) -> tuple[float, float] | None:
    ...
    def _compute_direction_labels(self, dataset: Any) -> dict[str, str] | None:
        """
    ...
    def set_image(self, image: Image.Image, preserve_view: bool = False, apply_inversion: bool | None = None) -> None:
        ...
        # qimage.copy() deep-copy that was previously needed to prevent a
        # segfault when Python GC'd the temporary bytes object. (P1.8)
        self._display_bytes_ref = image.tobytes()

        if image.mode == 'L':
            bytes_per_line = image.width * 1
    ...
    def compute_fit_zoom(self) -> float | None:
        """
        Zoom factor that fits the current image item into the viewport (rotation-aware).

        Returns:
            Positive fit zoom, or None if the image or viewport is unavailable or invalid.
        """
        if self.image_item is None:
            return None
        ...
        return max(1e-6, fit_zoom)

    def is_effectively_fit_and_centered(
        self,
        zoom_rel_tol: float = 1e-4,
        center_px_tol: float = 2.0,
    ) -> bool:
        """
        True if zoom matches fit-to-view (within tolerance) and the image center
        lies under the viewport center (within a few viewport pixels).

        Used to skip redundant ``fit_to_view`` when the viewport size did not change
        and the view is already in the default fit+centered state.
        """
        fit_z = self.compute_fit_zoom()
        if fit_z is None or self.current_zoom <= 0:
            return False
        if abs(self.current_zoom - fit_z) / fit_z > zoom_rel_tol:
            return False
        vc = self.get_viewport_center_scene()
        if vc is None or self.image_item is None:
            return False
        ic = self.image_item.boundingRect().center()
        p0 = QPointF(self.mapFromScene(vc))
        p1 = QPointF(self.mapFromScene(ic))
        dx = p0.x() - p1.x()
        dy = p0.y() - p1.y()
        return (dx * dx + dy * dy) ** 0.5 <= center_px_tol
    ...
    def fit_to_view(self, center_image: bool = False) -> None:
        ...
        scene_rect = self.image_item.boundingRect()  # type: ignore[union-attr]
    ...
    def get_viewport_center_scene(self) -> QPointF | None:
        """
    ...
    def _extract_image_region(self, center_x: float, center_y: float, size: int, zoom_factor: float) -> QPixmap | None:
        """
    ...
    def _render_scene_region(self, center_x: float, center_y: float, size: float, zoom_factor: float) -> QPixmap | None:
        """
        ...
        try:
            ...
            if len(array_shape) == 4:
                ...
                if is_color and array_shape[2] == samples_per_pixel:
                    ...
                    # Single-frame grayscale or ambiguous 3-D shape: keep the full
                    # array and let the shape checks below decide grayscale vs color.
                    frame_array = pixel_array
            else:
            ...
            if len(frame_array.shape) == 2:
                ...
                if use_rescaled:
                    slope, intercept, _ = _get_rescale_parameters(dataset)
