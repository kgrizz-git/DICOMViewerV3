...
Inputs (via callbacks set by the app):
    ...
    - get_layout_mode(): str in standard + 3-pane asymmetric modes.
...
from utils.config.layout_config import ALL_LAYOUT_MODES, THREE_PANE_LAYOUT_MODES
...
class WindowSlotMapWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.get_slot_to_view: Callable[[], list[int]] | None = None
        self.get_layout_mode: Callable[[], str] | None = None
        self.get_focused_view_index: Callable[[], int] | None = None
        self.get_thumbnail_for_view: Callable[[int], QPixmap | None] | None = None
        self.get_accent_id: Callable[[], str] | None = None
    ...
    def set_callbacks(
        self,
        get_slot_to_view: Callable[[], list[int]] | None = None,
        get_layout_mode: Callable[[], str] | None = None,
        get_focused_view_index: Callable[[], int] | None = None,
        get_thumbnail_for_view: Callable[[int], QPixmap | None] | None = None,
        get_accent_id: Callable[[], str] | None = None,
    ) -> None:
        ...
        self.get_accent_id = get_accent_id
        self.update()
    ...
    def _compute_displayed_slots(self, slot_to_view: list[int]) -> list[int]:
        ...
        if mode == "1+2B":
            return [0, 1, 2]
        if mode in THREE_PANE_LAYOUT_MODES:
            return [0, 1, 2]
        return []
    ...
    def paintEvent(self, event) -> None:  # type: ignore[override]
        ...
        accent_id = self.get_accent_id() if self.get_accent_id is not None else "blue"
        focused_border_color = get_focus_border_color(accent_id)
