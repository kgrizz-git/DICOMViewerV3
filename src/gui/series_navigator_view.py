...
class SeriesThumbnail(QFrame):
    ...
    def __init__(self, series_uid: str, series_number: int, thumbnail_image: Image.Image | None, study_uid: str = "", parent=None,
                 display_label: str | None = None, target_slice_index: int | None = None,
        ...
        # When True (default), always paint a compact slice/frame count badge on the thumbnail.
    ...
    def update_thumbnail_image(self, image: Image.Image | None) -> None:
        """Replace the thumbnail image and repaint.

        Called by deferred thumbnail generation after the initial placeholder
        layout has already been shown to the user.

        Args:
            image: New PIL Image to display, or None to keep the placeholder.
        """
    ...
    def set_show_slice_frame_count_badge(self, show: bool) -> None:
        """Show or hide the bottom slice/frame count badge (navigator setting)."""
        self._show_slice_frame_count_badge = bool(show)
        self.update()

    def _get_multiframe_indicator_text(self) -> str:
        """
        Compact slice/frame summary for the bottom badge.

        When ``_show_slice_frame_count_badge`` is True (default): always show a
        short label — instance count as slices when single-frame, ``Nf`` for
        single-instance multi-frame, or ``Ni×Nf`` when both exceed 1.

        When False: preserve legacy behaviour (badge only for multi-frame).
        """
        ic = self._instance_count
        mf = self._max_frame_count
    ...
    def paintEvent(self, event) -> None:
        ...
        if indicator_text:
            ...
            # Text only: full-opacity yellow (same size/placement as before; no badge box).
