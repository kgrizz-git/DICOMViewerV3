...
Inputs:
    - User interactions (play/pause toggle, stop, speed, loop)
...
class CineControlsWidget(QWidget):
    ...
    Features:
    - Play/Pause toggle (one button: Play when idle, Pause while playing)
    - Stop button
    ...
    # Signals
    play_requested = Signal()  # Emitted when play is requested (toggle shows Play)
    pause_requested = Signal()  # Emitted when pause is requested (toggle shows Pause)
    ...
    def __init__(self, parent=None):
        ...
        # Loop bounds state
        self.loop_start_frame: int | None = None
        self.loop_end_frame: int | None = None
    ...
    def _create_ui(self) -> None:
        ...
        # Control buttons row: play/pause, stop, loop
        buttons_layout = QHBoxLayout()
        buttons_layout.setSpacing(5)

        self.play_pause_button = QPushButton("▶ Play")
        self.play_pause_button.setToolTip("Play cine loop")
    ...
    def _on_play_pause_clicked(self) -> None:
        """Toggle: pause if playing, otherwise play."""
        if self._is_playing:
    ...
    def _set_controls_enabled(self, enabled: bool) -> None:
        ...
        if enabled:
            self._refresh_play_pause_appearance()

    def _refresh_play_pause_appearance(self) -> None:
        """Update play/pause button label and tooltip from ``_is_playing``."""
        if self._is_playing:
            self.play_pause_button.setText("⏸ Pause")
            self.play_pause_button.setToolTip("Pause cine playback")
        else:
    ...
    def update_playback_state(self, is_playing: bool) -> None:
        """
        Sync the play/pause toggle with playback state from CinePlayer.
    ...
    def _on_frame_slider_context_menu(self, position) -> None:
        ...
        # For horizontal slider, map x coordinate to value range
