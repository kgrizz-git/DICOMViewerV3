"""
Batch ACR MRI (pylinac) series-selection dialog.

Checkbox list of loaded MR series plus an "Add folder..." affordance,
per PYLINAC_ACR_FULL_METRICS_EXPORT_AND_MRI_BATCH_PLAN.md P4-M2. Returns
parallel ``(requests, labels)`` lists consumed by the MRI batch flow
(P4-M3 wires the menu/facade). Shared MRI options come from the existing
``prompt_acr_mri_options`` dialog; the ``stamp_mri_batch_options`` helper
applies them to each request via ``dataclasses.replace`` so M3 does not
grow ``qa_app_facade.py``.
...
def _series_label(ds: Dataset, series_key: str) -> str:
    ...
    to the composite series key when both tags are empty.
    """
...
def build_mri_series_entries(organizer: Any) -> list[tuple[str, str, str]]:
    """
    Enumerate MR series from the organizer for the selection list.

    Filters ``organizer.get_series_list()`` to series whose first dataset has
    ``Modality == "MR"``. Reading ``Modality`` / ``SeriesDescription`` /
...
def prompt_mri_batch_series_selection(
    parent: QWidget | None,
    organizer: Any,
    ...
    *,
    ...
    """
    Show the batch series-selection dialog for ACR MRI.
    ...
    dialog.setWindowTitle("ACR MRI Batch Analysis — Select Series")
    ...
    layout.addWidget(
        QLabel(
            "Check the MR series to include in this batch run. "
            "One ACR MRI options set applies to every series."
        )
    )
    ...
    if not entries:
        layout.addWidget(QLabel("No loaded MR series were found."))
...
def stamp_mri_batch_options(
    requests: list[QARequest],
    *,
    echo_number: int | None,
    check_uid: bool,
    origin_slice: int | None,
    scan_extent_tolerance_mm: float,
    vanilla_pylinac: bool,
    ...
    """
    Stamp shared MRI options onto each request via ``dataclasses.replace``.

    Mirrors the per-request options block the facade builds after
    ``prompt_acr_mri_options`` returns (see ``open_acr_mri_analysis``).
    Multi-series batch is **not** compare mode, so ``compare_request``/
    ``MRICompareRequest`` is intentionally absent here.
    ...
    return [
        replace(
            ...
            scan_extent_tolerance_mm=float(scan_extent_tolerance_mm),
            vanilla_pylinac=vanilla_pylinac,
            embed_module_images_in_xlsx=embed_module_images_in_xlsx,
            low_contrast_method=low_contrast_method,
            low_contrast_visibility_threshold=float(low_contrast_visibility_threshold),
            low_contrast_visibility_sanity_multiplier=float(
