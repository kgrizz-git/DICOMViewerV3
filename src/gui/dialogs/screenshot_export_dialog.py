...
from gui.export_rendering import effective_scale_for_image
...
class ScreenshotExportDialog(QDialog):
    ...
    def __init__(
        ...
        config_manager=None,
        multi_window_layout: Any | None = None,
        parent=None,
    ):
    ...
    def _grab_viewport(self, view_idx: int) -> QPixmap | None:
    ...
    def _viewport_for_view(self, view_idx: int) -> Any | None:
    ...
    def _scale_pixmap_for_export(self, pm: QPixmap) -> tuple[QPixmap, str | None]:
        """
    ...
    def _grab_viewport_for_composite_cell(self, view_idx: int) -> tuple[QPixmap | None, QColor]:
        """
    ...
    def _export_separate(self, prefix: str, ext: str, selected: list[int]) -> tuple[int, str | None, list[str]]:
        saved = 0
        notes: list[str] = []
        for i in selected:
    ...
    def _export_composite_tiled_fallback(self, prefix: str, ext: str) -> tuple[int, str | None, list[str]]:
        """
        ...
        col_w = [0] * cols
        row_h = [0] * rows
        cell_pix: dict[tuple[int, int], QPixmap | None] = {}
    ...
    def _scale_qimage_for_export(self, img: QImage) -> tuple[QImage, str | None]:
    ...
    def _export_composite(self, prefix: str, ext: str) -> tuple[int, str | None, list[str]]:
    ...
    def _export_full_window(self, prefix: str, ext: str) -> tuple[int, str | None, list[str]]:
        _parent = self.parent()
        if not isinstance(_parent, QWidget):
