...
class SrTreeModel(QAbstractItemModel):
    ...
    def parent(self, index: QModelIndex | QPersistentModelIndex) -> QModelIndex:  # pyright: ignore[reportIncompatibleMethodOverride]
...
class StructuredReportBrowserDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        if self._dose_summary is not None:
            ...
            self._summary_table.setColumnWidth(0, 220)
        ...
        if main_window is not None:
            _sig = getattr(main_window, "privacy_view_toggled", None)
            if _sig is not None:
                _sig.connect(self._on_privacy_toggled)
    ...
    def _populate_summary_table(self) -> None:
        ...
        rows = [
            ...
            ("Parser node cap reached", fmt(s.parse_node_cap_hit)),
        ]
