...
class OverlayConfigDialog(QDialog):
    ...
    - Live overlay preview: every tag addition, removal, reorder, or modality
      switch updates ConfigManager's in-memory config and is reflected on the
      overlay while the dialog remains open. Disk persistence is deferred to OK.
    ...
    Cancel behaviour: on reject() all modalities are restored in memory to the
    snapshot captured at dialog construction and config_changed is emitted once
    so the overlay reverts.
    """
    ...
    def __init__(self, config_manager: ConfigManager, parent: QWidget | None = None, initial_modality: str | None = None):
        """
    ...
    def _on_detail_mode_combo_changed(self, _index: int) -> None:
        """Update overlay mode in memory and refresh live preview."""
        mode = self.detail_mode_combo.currentData()
        if mode not in ("minimal", "detailed", "hidden"):
            return
        self.config_manager.set_overlay_mode(mode, persist=False)
    ...
    def _create_ui(self) -> None:
        ...
        header_row.addWidget(QLabel("Default (Spacebar):"))
    ...
    def _create_corner_widget(self, corner_name: str) -> QWidget:
        ...
        def sel_changed(cn=corner_name):
            return self._on_corner_selection_changed(cn)
    ...
    def _on_modality_changed(self, modality: str) -> None:
        """Qt slot wrapper that logs unexpected failures with context."""
        try:
    ...
    def _commit_current_modality_to_config(self) -> None:
        ...
        if self.current_modality in self.modality_configs:
            self.config_manager.set_overlay_tags(
                ...
                self.modality_configs[self.current_modality],
                persist=False,
            )
    ...
    def _commit_current_modality_detailed_to_config(self) -> None:
        ...
        if self.current_modality in self.modality_configs_detailed:
            self.config_manager.set_overlay_tags_detailed_extra(
                self.current_modality,
                self.modality_configs_detailed[self.current_modality],
                persist=False,
            )
    ...
    def _apply_configuration(self) -> None:
        ...
        for modality, config in self.modality_configs.items():
            self.config_manager.set_overlay_tags(modality, config, persist=False)
        for modality, config in self.modality_configs_detailed.items():
            self.config_manager.set_overlay_tags_detailed_extra(
                modality, config, persist=False
            )
    ...
    def reject(self) -> None:
        ...
        self.config_manager.set_overlay_mode(self._original_overlay_mode, persist=False)
        self.config_changed.emit()
        super().reject()
