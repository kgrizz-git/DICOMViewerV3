...
def show_image_background_context_menu_on_right_release(viewer: Any, event: Any) -> None:
    ...
    if not is_roi_item and not is_measurement_item:
        ...
        if hasattr(viewer, 'get_window_level_presets_callback') and viewer.get_window_level_presets_callback:
            ...
            if presets and len(presets) >= 1:  # Show menu even with single preset
                ...
                current_index = 0
                if hasattr(viewer, 'get_current_preset_index_callback') and viewer.get_current_preset_index_callback:
                ...
                for idx, (wc, ww, is_rescaled, name) in enumerate(presets):
                    ...
                    action.triggered.connect(lambda checked, i=idx: viewer.window_level_preset_selected.emit(i))
        ...
        # else:
        #     print(f"[DEBUG-WL-PRESETS] ImageViewer context menu: Callback missing or not set (hasattr={hasattr(viewer, 'get_window_level_presets_callback')}, callback={getattr(viewer, 'get_window_level_presets_callback', None)})")
        ...
        aip_action.triggered.connect(
            lambda: viewer.projection_type_changed.emit("aip")
        )
        ...
        mip_action.triggered.connect(
            lambda: viewer.projection_type_changed.emit("mip")
        )
        ...
        minip_action.triggered.connect(
            lambda: viewer.projection_type_changed.emit("minip")
        )
        ...
        use_raw_action.setChecked(not viewer.use_rescaled_values)  # Checked when using raw values
        ...
        use_rescaled_action.setChecked(viewer.use_rescaled_values)  # Checked when using rescaled values
