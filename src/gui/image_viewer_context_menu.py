...
QMenu construction for the image background, shown on right-release. See
``image_viewer_item_context_menu`` for the ROI / measurement / annotation /
crosshair menus shown immediately on right-press.
...
def show_image_background_context_menu_on_right_release(viewer: Any, event: Any) -> None:
    ...
    is_roi_item, is_measurement_item = _classify_right_release_pick(viewer, scene_pos)
    ...
    if not is_roi_item and not is_measurement_item:
        ...
        _add_navigation_section(viewer, context_menu)
        _add_tools_section(viewer, context_menu)
        _add_annotations_section(viewer, context_menu)
        _add_display_section(viewer, context_menu)
        _add_window_level_section(viewer, context_menu)
        _add_cine_section(viewer, context_menu)
        _add_info_file_section(viewer, context_menu)

        context_menu.exec(event.globalPosition().toPoint())


def _classify_right_release_pick(viewer: Any, scene_pos: Any) -> tuple[bool, bool]:
    """Classify the item under a right-release as ``(is_roi_item, is_measurement_item)``."""
    ...
    return is_roi_item, is_measurement_item


def _add_navigation_section(viewer: Any, context_menu: QMenu) -> None:
    """Reset/orientation/layout/swap/series/MPR/3D actions."""
    # ── Navigation / View ─────────────────────────────────────────────
...
def _add_tools_section(viewer: Any, context_menu: QMenu) -> None:
    """Interaction-mode tools submenu (Pan/Zoom/ROI/Measure/Annotations)."""
...
def _add_annotations_section(viewer: Any, context_menu: QMenu) -> None:
    """Annotation options / delete ROIs / clear measurements / export stats."""
    # ── Annotations ► ─────────────────────────────────────────────────
...
def _add_display_section(viewer: Any, context_menu: QMenu) -> None:
...
def _add_window_level_section(viewer: Any, context_menu: QMenu) -> None:
    """Window/level presets, invert, raw/rescaled, scroll wheel, combine slices."""
    # ── Window / Level ────────────────────────────────────────────────
    context_menu.addSeparator()
...
def _add_cine_section(viewer: Any, context_menu: QMenu) -> None:
    """Cine play/pause/stop/loop actions (only when cine controls are enabled)."""
    # ── Cine ─────────────────────────────────────────────────────────
...
def _add_info_file_section(viewer: Any, context_menu: QMenu) -> None:
    """Histogram / Structured Report / DICOM info / reveal-in-explorer actions."""
    # ── Info / File ───────────────────────────────────────────────────
    context_menu.addSeparator()
