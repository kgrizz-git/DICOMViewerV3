...
This module manages the layout of multiple subwindows (1x1, 1x2, 2x1, 2x2, and
four asymmetric 3-pane modes) for displaying different series/slices simultaneously.

Inputs:
    - Layout mode selection (1x1, 1x2, 2x1, 2x2, 1+2R, 2L+1, 2T+1, 1+2B)
    - Focus change requests
    - Series/slice assignment requests

Outputs:
    - Layout of subwindows (three visible + one hidden in 3-pane modes)
...
from utils.config.layout_config import (
    ALL_LAYOUT_MODES,
    THREE_PANE_CYCLE_ORDER,
    THREE_PANE_LAYOUT_MODES,
    TWO_PANE_LAYOUT_MODES,
    LayoutMode,
)
from utils.debug_flags import DEBUG_LAYOUT, DEBUG_RESIZE


def _is_three_pane_mode(layout_mode: str) -> bool:
    """Return True when ``layout_mode`` is one of the four asymmetric 3-pane modes."""
    return layout_mode in THREE_PANE_LAYOUT_MODES
...
class MultiWindowLayout(QWidget):
    ...
    def __init__(self, parent=None, config_manager=None):
        ...
        self.focused_subwindow: SubWindowContainer | None = None
        ...
        # Layout widget
        self.layout_widget: QWidget | None = None
        self.layout_manager: QGridLayout | None = None

        # Last layout before switching to 1x1 (for double-click-in-1x1 revert)
        self._last_layout_before_1x1: LayoutMode | None = None
    ...
    def set_layout(self, layout_mode: LayoutMode) -> None:
        ...
        Args:
            layout_mode: Layout mode (see ``ALL_LAYOUT_MODES`` in ``layout_config``).
        """
        if layout_mode not in ALL_LAYOUT_MODES:
            return
        ...
        if (
            ...
            and layout_mode != "1x1"
            and len(self.subwindows) >= num_subwindows
        ):
        ...
        # Persist last 2-pane / 3-pane choice for keyboard shortcuts (key 2 / 3).
        if self.config_manager is not None:
            if layout_mode in TWO_PANE_LAYOUT_MODES and hasattr(
                self.config_manager, "set_last_two_pane_layout"
            ):
                try:
                    self.config_manager.set_last_two_pane_layout(layout_mode)
                except Exception:
                    pass
            if _is_three_pane_mode(layout_mode) and hasattr(
                self.config_manager, "set_last_three_pane_layout"
            ):
                try:
                    self.config_manager.set_last_three_pane_layout(layout_mode)
                except Exception:
                    pass
    ...
    def _get_num_subwindows(self, layout_mode: LayoutMode) -> int:
        ...
        if layout_mode == "1x1":
            return 1
        elif layout_mode == "1x2" or layout_mode == "2x1":
            return 2
        elif layout_mode == "2x2":
            return 4
        if _is_three_pane_mode(layout_mode):
            return 4
        return 1

    def cycle_three_pane_layout(self) -> LayoutMode:
        """
        Cycle asymmetric 3-pane modes: 1+2R → 2L+1 → 2T+1 → 1+2B → wrap.

        From 1×1 / 2×2 / 2-pane, enters last-used 3-pane from config or ``1+2R``.
        """
        if self.current_layout in THREE_PANE_CYCLE_ORDER:
            idx = THREE_PANE_CYCLE_ORDER.index(self.current_layout)
            next_mode = THREE_PANE_CYCLE_ORDER[(idx + 1) % len(THREE_PANE_CYCLE_ORDER)]
        else:
            next_mode = "1+2R"
            if self.config_manager is not None and hasattr(
                self.config_manager, "get_last_three_pane_layout"
            ):
                try:
                    next_mode = cast(LayoutMode, self.config_manager.get_last_three_pane_layout())
                except Exception:
                    pass
        self.set_layout(next_mode)
        return next_mode

    def toggle_two_pane_layout(self) -> LayoutMode:
        """
        Toggle 1×2 ↔ 2×1. From a 3-pane mode, switch to last-used 2-pane (config).
        """
        if _is_three_pane_mode(self.current_layout):
            mode: LayoutMode = "1x2"
            if self.config_manager is not None and hasattr(
                self.config_manager, "get_last_two_pane_layout"
            ):
                try:
                    mode = self.config_manager.get_last_two_pane_layout()
                except Exception:
                    pass
            self.set_layout(mode)
            return mode
        if self.current_layout == "1x2":
            self.set_layout("2x1")
            return "2x1"
        if self.current_layout == "2x1":
            self.set_layout("1x2")
            return "1x2"
        mode = "1x2"
        if self.config_manager is not None and hasattr(
    ...
    def _get_first_visible_container(self, layout_mode: LayoutMode) -> SubWindowContainer | None:
        """
        ...
        if layout_mode == "2x2":
            ...
            return self.subwindows[0] if self.subwindows else None
        if _is_three_pane_mode(layout_mode) and len(self.subwindows) >= 4:
            visible_views = self._three_pane_visible_view_indices(layout_mode)
            focused_idx = self._get_focused_view_index()
            if focused_idx in visible_views:
                return self.subwindows[focused_idx]
            if visible_views:
                return self.subwindows[visible_views[0]]
        return self.subwindows[0] if self.subwindows else None

    def _three_pane_visible_view_indices(self, layout_mode: str) -> list[int]:
        """
        View indices (0–3) shown on screen for a 3-pane mode (T0 slot table).

        Hidden slot retains series assignment but is not placed in the grid.
        """
        stv = self.slot_to_view
        if layout_mode == "1+2R":
            return [stv[0], stv[1], stv[2]]
        if layout_mode == "2L+1":
            return [stv[1], stv[2], stv[3]]
        if layout_mode == "2T+1":
            return [stv[0], stv[1], stv[2]]
        if layout_mode == "1+2B":
            return [stv[0], stv[1], stv[2]]
    ...
    def _three_pane_hidden_view_index(self, layout_mode: str) -> int:
        """View index hidden (not destroyed) in the given 3-pane mode."""
        stv = self.slot_to_view
        if layout_mode == "2L+1":
            return stv[0]
        return stv[3]

    def _arrange_three_pane(self, layout_mode: str, grid: QGridLayout) -> None:
        """
        Place three visible subwindows in a 2×2 grid with row/col spans (T0 table).

        Stretch factors favor the large pane (2:1). Fourth view is hidden, not removed.
        """
        if len(self.subwindows) < 4:
            return
        stv = self.slot_to_view
        hidden_idx = self._three_pane_hidden_view_index(layout_mode)

        def show_at(view_idx: int, row: int, col: int, row_span: int, col_span: int) -> None:
            container = self.subwindows[view_idx]
            container.show()
            grid.addWidget(container, row, col, row_span, col_span)

        if layout_mode == "1+2R":
            show_at(stv[0], 0, 0, 2, 1)
            show_at(stv[1], 0, 1, 1, 1)
            show_at(stv[2], 1, 1, 1, 1)
            grid.setColumnStretch(0, 2)
            grid.setColumnStretch(1, 1)
            grid.setRowStretch(0, 1)
            grid.setRowStretch(1, 1)
        elif layout_mode == "2L+1":
            show_at(stv[1], 0, 0, 1, 1)
            show_at(stv[2], 1, 0, 1, 1)
            show_at(stv[3], 0, 1, 2, 1)
            grid.setColumnStretch(0, 1)
            grid.setColumnStretch(1, 2)
            grid.setRowStretch(0, 1)
            grid.setRowStretch(1, 1)
        elif layout_mode == "2T+1":
            show_at(stv[0], 0, 0, 1, 2)
            show_at(stv[1], 1, 0, 1, 1)
            show_at(stv[2], 1, 1, 1, 1)
            grid.setRowStretch(0, 2)
            grid.setRowStretch(1, 1)
            grid.setColumnStretch(0, 1)
            grid.setColumnStretch(1, 1)
        elif layout_mode == "1+2B":
            show_at(stv[0], 0, 0, 1, 1)
            show_at(stv[1], 0, 1, 1, 1)
            show_at(stv[2], 1, 0, 1, 2)
            grid.setRowStretch(0, 1)
            grid.setRowStretch(1, 2)
            grid.setColumnStretch(0, 1)
            grid.setColumnStretch(1, 1)

        self.subwindows[hidden_idx].hide()
    ...
    def _create_subwindow(self) -> SubWindowContainer:
        ...
        if self.config_manager is not None:
            container.get_accent_id = self.config_manager.get_accent
    ...
    def _arrange_subwindows(self, layout_mode: LayoutMode) -> None:
        ...
        if layout_mode == "1x2":
            ...
            expected_rows = 2
            expected_cols = 1
        elif layout_mode == "2x2" or _is_three_pane_mode(layout_mode):
            expected_rows = 2
            expected_cols = 2

        # Arrange based on layout mode (focus-based for 1x1, 1x2, 2x1; slot_to_view for 2x2 / 3-pane)
        if layout_mode == "1x1":
        ...
        elif _is_three_pane_mode(layout_mode):
            self._arrange_three_pane(layout_mode, grid)
        ...
        # Set stretch factors for expected rows and columns (3-pane sets asymmetric 2:1 in helper).
        if not _is_three_pane_mode(layout_mode):
    ...
    def get_focused_subwindow(self) -> SubWindowContainer | None:
        """
    ...
    def get_subwindow(self, index: int) -> SubWindowContainer | None:
        """
    ...
    def get_screenshot_grid_cells(self) -> list[tuple[int, int, int]]:
        ...
        if mode == "2x2":
            ...
            return out
        if mode == "1+2R":
            return [(0, 0, stv[0]), (0, 1, stv[1]), (1, 1, stv[2])]
        if mode == "2L+1":
            return [(0, 0, stv[1]), (1, 0, stv[2]), (0, 1, stv[3])]
        if mode == "2T+1":
            return [(0, 0, stv[0]), (1, 0, stv[1]), (1, 1, stv[2])]
        if mode == "1+2B":
            return [(0, 0, stv[0]), (0, 1, stv[1]), (1, 0, stv[2])]
        idx = self._get_focused_view_index()
        return [(0, 0, idx)]
    ...
    def get_revert_layout(self) -> LayoutMode:
        ...
        """
        if self._last_layout_before_1x1 in ALL_LAYOUT_MODES and self._last_layout_before_1x1 != "1x1":
    ...
    def swap_views(self, view_index_a: int, view_index_b: int) -> None:
        ...
        elif _is_three_pane_mode(self.current_layout):
            self._arrange_subwindows(self.current_layout)
