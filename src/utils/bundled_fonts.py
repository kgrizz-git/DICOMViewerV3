...
_SEMICOND_QT_FAMILY: dict[str, str] = {
...
_VARIANT_ATTRS: dict[str, tuple[int, bool]] = {
    "Thin":              (100, False),
...
def _fonts_dir() -> Path:
    ...
    if getattr(sys, "frozen", False):
        ...
        if meipass is not None:
            return Path(str(meipass)) / "resources" / "fonts"
...
def get_font_variants(family: str) -> list[str]:
...
def resolve_font(family: str, variant: str = DEFAULT_FONT_VARIANT) -> tuple[str, str]:
...
def get_variant_weight_italic(variant: str) -> tuple[int, bool]:
...
def make_qfont(family: str, variant: str, size: int) -> QFont:
...
def register_fonts_with_qt() -> None:
    ...
    try:
        ...
        for _family, variants in BUNDLED_FONTS.items():
            for _variant, relative in variants.items():
