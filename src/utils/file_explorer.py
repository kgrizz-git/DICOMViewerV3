...
def reveal_file_in_explorer(file_path: str) -> bool:
    ...
    try:
        if system == "Darwin":  # macOS
            ...
            # Pass a single Windows command line so Explorer receives the quoted
            # /select target correctly and does not fall back to Desktop.
            subprocess.run(f'explorer /select,"{abs_path}"', check=True)
            return True
