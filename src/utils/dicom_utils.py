...
def _get_functional_group_item(dataset: Dataset, sequence_keyword: str) -> Dataset | None:
    """Return the first item of *sequence_keyword* from functional groups, or None.

    Checks ``SharedFunctionalGroupsSequence`` first (more common for geometry
    shared across all frames), then ``PerFrameFunctionalGroupsSequence[0]``.
    """
    for fg_attr in ("SharedFunctionalGroupsSequence", "PerFrameFunctionalGroupsSequence"):
        fg_seq = getattr(dataset, fg_attr, None)
        if fg_seq and len(fg_seq) > 0:
            inner = getattr(fg_seq[0], sequence_keyword, None)
            if inner and len(inner) > 0:
                return inner[0]
    return None


def get_pixel_spacing(dataset: Dataset) -> tuple[float, float] | None:
    """
    Get pixel spacing from DICOM dataset.
    Checks multiple sources in priority order:
    1. Pixel Spacing (0028,0030) - primary
    2. Enhanced Multi-frame PixelMeasuresSequence in functional groups
    3. Imager Pixel Spacing (0018,1164) - fallback
    4. Calculate from Field of View + Matrix Size - fallback
    ...
    # Priority 2: Enhanced Multi-frame PixelMeasuresSequence
    try:
        pm = _get_functional_group_item(dataset, "PixelMeasuresSequence")
        if pm is not None:
    ...
    # Priority 3: Imager Pixel Spacing (0018,1164)
    try:
    ...
    # Priority 4: Calculate from Field of View + Matrix Size
...
def get_slice_thickness(dataset: Dataset) -> float | None:
    """
    Get slice thickness from DICOM dataset.

    Checks the top-level tag first, then falls back to
    ``PixelMeasuresSequence`` inside Enhanced Multi-frame functional groups.
    ...
    # Fallback: Enhanced Multi-frame PixelMeasuresSequence
    try:
        pm = _get_functional_group_item(dataset, "PixelMeasuresSequence")
        if pm is not None:
...
def pixels_to_mm(pixels: float, pixel_spacing: tuple[float, float] | None,
                 dimension: int = 0) -> float | None:
    """
...
def mm_to_pixels(mm: float, pixel_spacing: tuple[float, float] | None,
                 dimension: int = 0) -> float | None:
    """
...
def format_distance(pixels: float, pixel_spacing: tuple[float, float] | None = None,
                    dimension: int = 0) -> str:
    """
...
def get_image_position(dataset: Dataset) -> np.ndarray | None:
    """
    Get ImagePositionPatient from DICOM dataset.

    Falls back to ``PlanePositionSequence`` inside Enhanced Multi-frame
    ...
    # Fallback: Enhanced Multi-frame PlanePositionSequence
    try:
        pp = _get_functional_group_item(dataset, "PlanePositionSequence")
        if pp is not None:
...
def get_slice_location(dataset: Dataset) -> float | None:
    """
...
def get_image_orientation(dataset: Dataset) -> tuple[np.ndarray, np.ndarray] | None:
    """
    Get ImageOrientationPatient from DICOM dataset.

    Falls back to ``PlaneOrientationSequence`` inside Enhanced Multi-frame
    functional groups when the top-level tag is absent.
    ...
    # Fallback: Enhanced Multi-frame PlaneOrientationSequence
    try:
        po = _get_functional_group_item(dataset, "PlaneOrientationSequence")
        if po is not None:
            orient = getattr(po, "ImageOrientationPatient", None)
            if orient and len(orient) >= 6:
                row_cosine = np.array([float(orient[0]), float(orient[1]), float(orient[2])])
                col_cosine = np.array([float(orient[3]), float(orient[4]), float(orient[5])])
                return (row_cosine, col_cosine)
...
def canonical_dicom_tag_string(tag_str: str) -> str | None:
    """
