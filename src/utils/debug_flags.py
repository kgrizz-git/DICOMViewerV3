...
Discoverability (agents and contributors):
    - AGENTS.md — repository map row and conventions bullet
    - dev-docs/HARNESS.md — "Debugging (console traces)" section
    - dev-docs/README.md — developer index link to this file
    CI rejects any DEBUG_* left True (.github/workflows/security-checks.yml).
...
When adding new tracing: define a new DEBUG_* constant here (default False),
document which modules it affects in the comment above the constant, and gate
prints with ``if DEBUG_*:`` — do not leave ungated diagnostic prints in src/.
"""
...
# Enables 3D volume rendering tracing (volume build, VTK pipeline setup, transfer functions).
# Affects: volume_renderer.py, volume_render_dialog.py, volume_viewer_widget.py,
#          volume_render_eligibility.py, gui/volume_render_facade.py.
DEBUG_VOLUME_3D: bool = False

import os

# Enables startup and section performance logging via [PERF] log lines.
# Activated by setting the environment variable DICOM_PERF_LOG=1.
# Affects: main.py (startup timing), utils/perf_timer.py (section timers).
PERF_LOG: bool = os.environ.get("DICOM_PERF_LOG", "") == "1"
