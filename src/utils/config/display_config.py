...
from utils.accent_presets import ACCENT_PRESETS
...
class DisplayConfigMixin:
    ...
    def get_accent(self) -> str:
        """
        Get the accent colour preset ID.
    ...
    def set_accent(self, accent_id: str) -> None:
        """
        Set the accent colour preset.

        Args:
            accent_id: One of the keys in ``utils.accent_presets.ACCENT_PRESETS``.
                       Silently ignores unknown values.
        """
        if accent_id in ACCENT_PRESETS:
            self._config()["accent"] = accent_id
            self._save_config()
    ...
    def get_smooth_image_when_zoomed(self) -> bool:
        ...
        """
        return self._config().get("smooth_image_when_zoomed", True)
    ...
    def get_slice_slider_placement(self) -> str:
        """Return the preferred in-window slider placement edge."""
        return normalize_slider_placement(
            self._config().get("slice_slider_placement", DEFAULT_SLIDER_PLACEMENT)
        )

    def set_slice_slider_placement(self, placement: str) -> None:
        """Persist the preferred in-window slider placement edge."""
        self._config()["slice_slider_placement"] = normalize_slider_placement(placement)
        self._save_config()

    def get_slice_slider_direction(self) -> str:
        """Return where the first slice/frame appears on the slider."""
        return normalize_slider_direction(
            self._config().get("slice_slider_direction", DEFAULT_SLIDER_DIRECTION)
        )

    def set_slice_slider_direction(self, direction: str) -> None:
        """Persist where the first slice/frame appears on the slider."""
        self._config()["slice_slider_direction"] = normalize_slider_direction(direction)
        self._save_config()
    ...
    def get_histogram_window_geometry(self) -> tuple[int, int, int, int] | None:
        """
    ...
    def get_toolbar_label_style(self) -> str:
        """
        Get the toolbar button label style preference.
    ...
    def set_toolbar_label_style(self, style: str) -> None:
        """
        Set the toolbar button label style preference.

        Args:
            style: One of ``"icon_only"``, ``"text_under_icon"``, ``"text_only"``.
                   Silently ignores unknown values.
        """
        if style in ("icon_only", "text_under_icon", "text_only"):
            self._config()["toolbar_label_style"] = style
            self._save_config()
    ...
    def get_wl_user_presets(self) -> list[dict[str, Any]]:
        """
        Return user-defined W/L presets.

        Each entry is a dict with keys: ``name`` (str), ``modality`` (str, e.g. "CT" or "ANY"),
        ``center`` (float), ``width`` (float), ``is_rescaled`` (bool, True = calibrated/HU).
        """
        return list(self._config().get("wl_user_presets", []))

    def set_wl_user_presets(self, presets: list[dict[str, Any]]) -> None:
        """Persist user-defined W/L presets."""
        self._config()["wl_user_presets"] = list(presets)
        self._save_config()

    def get_volume_3d_user_presets(self) -> list[dict[str, Any]]:
        """
        Return user-saved 3D volume render presets.

        Each entry: ``name``, ``base_preset`` (built-in TF name), ``opacity`` (0–100),
        ``window``, ``level``, ``threshold`` (−500–500).
        """
        return list(self._config().get("volume_3d_user_presets", []))

    def set_volume_3d_user_presets(self, presets: list[dict[str, Any]]) -> None:
        """Persist user-saved 3D volume render presets."""
        self._config()["volume_3d_user_presets"] = list(presets)
        self._save_config()
