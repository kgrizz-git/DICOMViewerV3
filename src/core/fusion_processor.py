...
class FusionProcessor:
    ...
    AVAILABLE_COLORMAPS: ClassVar[list[str]] = [
    ...
    def create_fusion_image(
        ...
        colormap: str = 'hot',
        threshold: float = 0.0,
        base_wl: tuple[float, float] | None = None,
        overlay_wl: tuple[float, float] | None = None,
        base_pixel_spacing: tuple[float, float] | None = None,
        overlay_pixel_spacing: tuple[float, float] | None = None,
        translation_offset: tuple[float, float] | None = None,
        ...
        overlay_mask_3d = np.broadcast_to(
            overlay_mask[..., np.newaxis], (*overlay_mask.shape, 3)
        )  # no copy needed: used read-only in alpha_mask multiply (P1.3)
