...
def read_slice_location(dataset: Dataset) -> float | None:
...
def read_pixel_spacing(dataset: Dataset) -> tuple[float, float] | None:
...
def translation_offset_pixels_from_ipps(
    ...
    When *iop* (ImageOrientationPatient, 6 floats) is supplied the 3-D
    physical offset is projected onto the row and column cosine vectors,
    which is correct for oblique orientations.  Without *iop* the function
    falls back to the simpler X/Y-only formula (correct for pure axial).
    """
    offset_mm = (
        overlay_ipp[0] - base_ipp[0],
        overlay_ipp[1] - base_ipp[1],
        overlay_ipp[2] - base_ipp[2],
    )

    if iop is not None and len(iop) >= 6:
        row_cos = (float(iop[0]), float(iop[1]), float(iop[2]))
        col_cos = (float(iop[3]), float(iop[4]), float(iop[5]))
        # Project 3-D offset onto in-plane directions
        along_col = sum(o * r for o, r in zip(offset_mm, row_cos, strict=False))
        along_row = sum(o * c for o, c in zip(offset_mm, col_cos, strict=False))
        offset_px_x = along_col / base_pixel_spacing[1]
        offset_px_y = along_row / base_pixel_spacing[0]
    else:
        offset_px_x = offset_mm[0] / base_pixel_spacing[1]
        offset_px_y = offset_mm[1] / base_pixel_spacing[0]
