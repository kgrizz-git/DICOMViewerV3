...
class CineAppFacade:
    ...
    __slots__ = ("__weakref__", "_app")  # pyright: ignore[reportUninitializedInstanceVariable]
    ...
    def on_cine_playback_state_changed(self, is_playing: bool) -> None:
        """Sync cine controls, FPS readout, and toolbar icon with playback state."""
        app = self._app
        ...
        # Swap toolbar button icon between play and pause.
        action = getattr(app.main_window, "cine_play_pause_action", None)
        if action is not None:
            if is_playing:
                action.setIcon(getattr(app.main_window, "_cine_icon_pause", action.icon()))
                action.setToolTip("Pause cine loop  (Ctrl+Space)")
            else:
                action.setIcon(getattr(app.main_window, "_cine_icon_play", action.icon()))
                action.setToolTip("Play cine loop  (Ctrl+Space)")
