...
class PresentationStateHandler:
    ...
    def parse_presentation_state(self, dataset: Dataset) -> dict[str, Any]:
        ...
        if hasattr(dataset, 'GraphicAnnotationSequence'):
            ...
            if result['annotations']:
                [ann.get('type', 'UNKNOWN') for ann in result['annotations']]
    ...
    def parse_display_settings(self, dataset: Dataset) -> dict[str, Any]:
        ...
        try:
            # Window/Level settings (with Enhanced Multi-frame fallback)
            from core.dicom_window_level import get_window_level_from_dataset as _get_wl
            wl_center, wl_width, _ = _get_wl(dataset)
            if wl_center is not None:
                settings['window_center'] = self._coerce_float(wl_center)
            if wl_width is not None:
                settings['window_width'] = self._coerce_float(wl_width)
