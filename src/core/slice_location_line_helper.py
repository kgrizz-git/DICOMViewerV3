...
def get_slice_location_line_segments(
    ...
    other_indices: list[int] | None = None,
    mode: str = "middle",
) -> list[dict[str, Any]]:
    """
    ...
    for source_idx in source_indices:
        ...
        source_is_mpr = bool(
            hasattr(app, "subwindow_data")
            and app.subwindow_data.get(source_idx, {}).get("is_mpr", False)
        )
        ...
        if mode == "begin_end":
            ...
            if clipped is not None:
                c1, r1, c2, r2 = clipped
                lid = f"mpr_ref:middle:{source_idx}" if source_is_mpr else f"middle:{source_idx}"
                segments.append({
...
def _append_begin_end_segments(
    ...
    When is_mpr is True each id is prefixed with "mpr_ref:" so the manager
    can render them with a dashed pen.
    ...
    Args:
        ...
        is_mpr:       True when source_idx is an MPR reconstruction pane.
    """
    pfx = "mpr_ref:" if is_mpr else ""
    if thickness is None or thickness <= 0:
        ...
        if clipped is not None:
            ...
            segments.append({
                "source_idx": source_idx,
                "line_id": f"{pfx}middle:{source_idx}",
                "col1": c1,
                "row1": r1,
                "col2": c2,
                "row2": r2,
    ...
    if clipped_begin is not None:
        ...
        segments.append({
            "source_idx": source_idx,
            "line_id": f"{pfx}begin:{source_idx}",
            "col1": c1,
            "row1": r1,
            "col2": c2,
            "row2": r2,
...
def _get_image_size(app: Any, idx: int) -> tuple[float | None, float | None]:
    """
...
def _get_frame_of_reference_uid(app: Any, idx: int) -> str | None:
