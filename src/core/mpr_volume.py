...
class MprVolume:
    ...
    def from_datasets(
        ...
        for _i, plane in enumerate(initial_stack.planes):
        ...
        # Optimisation: skip rebuild when no duplicates were actually removed.
    ...
    def _deduplicate_sorted(
        ...
        for ds, pos in zip(sorted_datasets[1:], sorted_positions[1:], strict=False):
    ...
    def _build_sitk_image(
        ...
        stack: SliceStack,
        pixel_spacing_mm: tuple[float, float],
        averaged_arrays: dict[int, np.ndarray] | None = None,
    ) -> Any:
        """
