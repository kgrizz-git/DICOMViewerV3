...
class OverlayMatchResult(Enum):
    """
    Classification of a base-slice position relative to the overlay stack.

    Used by find_matching_slice_with_classification to communicate WHY a
    match was or was not found, so callers can show accurate status hints
    rather than silently showing base-only.

    Values:
        inside      – base slice is within the overlay stack (match found or
                      bracketing indices available).
        below_stack – base slice is caudal to the first overlay slice.
        above_stack – base slice is cranial to the last overlay slice.
        no_geometry – overlay locations could not be determined (empty list
                      or missing slice-location tags on the base slice).
    """
    inside = "inside"
    below_stack = "below_stack"
    above_stack = "above_stack"
...
class FusionHandler:
    ...
    def __init__(self):
        ...
        self.base_series_uid: str | None = None
        self.overlay_series_uid: str | None = None
        ...
        self._resampling_decision_cache: tuple[bool, str] | None = None
        self._resampling_decision_cache_key: tuple[str, str] | None = None
        ...
        self._alignment_cache: dict[tuple[str, str], dict[str, tuple[float, float] | None]] = {}

        # Track actual resampling mode used (True = 3D used, False = 2D fallback, None = not determined yet)
        self._actual_resampling_mode_used: bool | None = None
        # Store failure reason when 3D fails
        self._resampling_failure_reason: str | None = None

        # Last overlay match classification — updated by interpolate_overlay_slice
        # so callers can surface accurate status hints when overlay is absent.
        self._last_overlay_match_result: OverlayMatchResult | None = None
    ...
    def get_slice_location(self, dataset: Dataset) -> float | None:
        """
    ...
    def has_duplicate_locations(
        ...
        for _idx, location in locations:
            is_duplicate = False
    ...
    def find_matching_slice_with_classification(
        self,
        base_slice_idx: int,
        base_datasets: list[Dataset],
        overlay_datasets: list[Dataset],
    ) -> tuple[OverlayMatchResult, int | None, int | None]:
        """
        Find matching overlay slice(s) for a base slice and classify the result.

        Returns the position classification alongside the slice index pair so
        callers can distinguish "outside coverage" from "no geometry" and
        surface an appropriate status hint.

        Args:
            base_slice_idx: Index of base slice in base_datasets.
            base_datasets:  Ordered list of base-series DICOM datasets.
            overlay_datasets: Ordered list of overlay-series DICOM datasets.

        Returns:
            Tuple of (OverlayMatchResult, slice_idx_1, slice_idx_2) where:
            - ``OverlayMatchResult.inside``      – match found; indices are valid.
            - ``OverlayMatchResult.below_stack`` – base below first overlay slice;
              both indices are None.
            - ``OverlayMatchResult.above_stack`` – base above last overlay slice;
              both indices are None.
            - ``OverlayMatchResult.no_geometry`` – geometry missing; both None.

            When inside and an exact match is found: (inside, idx, None).
            When inside and interpolation is needed: (inside, lower_idx, upper_idx).
        """
        if base_slice_idx >= len(base_datasets):
            return (OverlayMatchResult.no_geometry, None, None)

        # Resolve base slice location.
        base_location = self.get_slice_location(base_datasets[base_slice_idx])
        if base_location is None:
            return (OverlayMatchResult.no_geometry, None, None)

        # Sorted (index, location) pairs for the overlay series.
        ...
        # Exact-match tolerance (0.01 mm — duplicate-detection level).
        tolerance = 0.01

        for i, (idx, location) in enumerate(overlay_locations):
            if abs(location - base_location) < tolerance:
                return (OverlayMatchResult.inside, idx, None)

            if location > base_location:
                if i == 0:
                    # Base is caudal to all overlay slices.
                    return (OverlayMatchResult.below_stack, None, None)
                # Return bracketing slices for linear interpolation.
                lower_idx = overlay_locations[i - 1][0]
                return (OverlayMatchResult.inside, lower_idx, idx)

        # Base is cranial to all overlay slices.
        return (OverlayMatchResult.above_stack, None, None)
    ...
    def find_matching_slice(
        ...
        Delegates to find_matching_slice_with_classification and strips the
        OverlayMatchResult for backward compatibility with existing callers.
        ...
        """
        _, idx1, idx2 = self.find_matching_slice_with_classification(
    ...
    def interpolate_overlay_slice(
        ...
        # Classify the base slice's position relative to the overlay stack.
        # This single call covers both the 3D and 2D paths:
        #   - below_stack / above_stack / no_geometry → no overlay possible;
        #     return None immediately and skip 3D (avoids SimpleITK edge-fill
        #     artefacts when the base slice is outside the overlay FOV — T5).
        #   - inside → overlay exists; proceed to 3D or 2D as configured.
        match_result, idx1, idx2 = self.find_matching_slice_with_classification(
            base_slice_idx, base_datasets, overlay_datasets
        )
        self._last_overlay_match_result = match_result
        ...
        if match_result in (
            ...
            # No overlay coverage — return None without attempting 3D or 2D.
            return None
        ...
        if use_3d:
            ...
            try:
                ...
                if resampled_slice is not None:
                    ...
                    print("Warning: 3D resampling returned None, falling back to 2D")
        ...
        # Re-use the classification result computed above — idx1/idx2 already
        # hold the correct bracketing indices when match_result is inside.
        if idx1 is None:
            return None
    ...
    def get_pixel_spacing(self, dataset: Dataset) -> tuple[float, float] | None:
        """
    ...
    def get_image_position_patient(self, dataset: Dataset) -> tuple[float, float, float] | None:
        """
    ...
    def calculate_translation_offset(
        ...
        iop = getattr(base_dataset, "ImageOrientationPatient", None)
        iop_tuple = tuple(float(v) for v in iop) if iop and len(iop) >= 6 else None
        offset_px_x, offset_px_y = fusion_io.translation_offset_pixels_from_ipps(
            base_ipp, overlay_ipp, base_pixel_spacing, iop=iop_tuple
        )
    ...
    def get_actual_resampling_mode_used(self) -> bool | None:
        """
    ...
    def get_resampling_failure_reason(self) -> str | None:
        """
    ...
    def clear_alignment_cache(self, series_uid: str | None = None) -> None:
        """
