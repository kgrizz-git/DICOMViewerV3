...
Sequence-valued tags (VR SQ) are included: a sequence is always a selectable row,
exporting a single summary cell. ``DeidentificationMethodCodeSequence``, and any
other sequence whose items carry CodeValue/CodeMeaning, is rendered by
``DICOMParser`` as a compact CID 7050 code summary; other SQ tags get a plain
item-count summary (``"3 item(s)"``). The export dialog's "Include sequences"
checkbox governs whether a sequence's *nested contents* also become their own
columns — not whether the sequence itself is offered. Explicit (group, element)
tuples cover tags when keyword lookup is unreliable.
...
_resolved_catalog: list[tuple[str, str, str, str]] | None = None
...
def _resolve_catalog() -> list[tuple[str, str, str, str]]:
    ...
    SQ-valued catalog tags (e.g. ``DeidentificationMethodCodeSequence``) are included
    unconditionally: a sequence is always a visible row now, valued with a summary, so
    a *missing* sequence must be offerable in the picker for the same reason any other
    missing standard tag is.

    Returns:
        Deduplicated catalog rows; private tags omitted.
    """
...
def supplement_export_tags_dict(tags: dict[str, Any]) -> None:
    ...
    Synthetic entries use an empty string value and is_private False. They carry the
    same tree fields real parser rows do — a synthetic tag is by definition absent from
    the dataset, so it is always a root-level row with no children.
    """
    for tag_str, keyword, name, vr in _resolve_catalog():
        if tag_str not in tags:
            tags[tag_str] = {
                ...
                "parent_key": None,
                "item_index": None,
                "row_kind": "sequence" if vr == "SQ" else "element",
            }
...
def _tag_from_identifier_string(s: str) -> BaseTag | None:
    """
...
def synthetic_tag_export_tree_entry(
    ...
    try:
        ...
        pass
    try:
        # Raises KeyError for any tag outside the standard dictionary — a private tag in
        # a saved preset, most of all. Unguarded, that took down the whole preset load.
        description = dictionary_description(tg) or ""
    except Exception:
        pass
    return key, {
        "tag": key,
        ...
        "name": description or name or key,
    }
