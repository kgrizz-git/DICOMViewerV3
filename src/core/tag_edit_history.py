...
class EditTagCommand(TagEditCommand):
    ...
    def __init__(self, dataset: Dataset, tag_identifier: str | tuple[int, int] | Any,
                 old_value: Any, new_value: Any, vr: str | None = None):
        """
        ...
        if hasattr(self.tag_identifier, 'group') and hasattr(self.tag_identifier, 'element'):
            ...
            if len(parts) == 2:
                try:
                    ...
                    raise ValueError(f"Invalid tag format: {self.tag_identifier}") from None
            else:
...
class TagEditHistoryManager:
    ...
    def _get_tag_string(self, command: EditTagCommand) -> str | None:
        """
    ...
    def get_original_value(self, dataset: Dataset, tag_str: str) -> Any | None:
        """
    ...
    def clear_edited_tags(self, dataset: Dataset | None = None) -> None:
        """
    ...
    def clear_history(self, dataset: Dataset | None = None) -> None:
        """
