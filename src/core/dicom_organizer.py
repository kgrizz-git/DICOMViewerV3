...
class DICOMOrganizer:
    ...
    def organize(self, datasets: list[Dataset], file_paths: list[str] | None = None) -> dict[str, dict[str, list[Dataset]]]:
        """
    ...
    def _organize_files_into_batch(
        ...
        file_paths_input: list[str] | None = None,
    ) -> tuple[
        ...
        study_dict: dict[str, dict[str, list[tuple[Dataset, str | None]]]] = defaultdict(
            lambda: defaultdict(list)
        )
        total_frame_wrappers = 0
        ...
        with perf_timer("first_paint.merge_batch.organize.scan_and_split"):
            for idx, dataset in enumerate(datasets):
                ...
                if sop_class_uid_str in (
                    GRAYSCALE_PRESENTATION_STATE_UID,
                    COLOR_PRESENTATION_STATE_UID,
                ):
                    study_uid = self._get_tag_value(dataset, "StudyInstanceUID", "")
                    if study_uid:
                ...
                if is_multiframe(dataset):
                    ...
                    total_frame_wrappers += num_frames
                    perf_mark(
                        "first_paint.merge_batch.organize.multiframe_detected",
                        dataset_index=idx,
                        frames=num_frames,
                    )
                    with perf_timer("first_paint.merge_batch.organize.create_frame_wrappers"):
        ...
        perf_mark(
            "first_paint.merge_batch.organize.scan_summary",
            input_datasets=len(datasets),
            frame_wrappers=total_frame_wrappers,
            studies=len(study_dict),
            series=sum(len(series_dict) for series_dict in study_dict.values()),
        )

        with perf_timer("first_paint.merge_batch.organize.sort_and_map"):
            for study_uid, series_dict in study_dict.items():
                ...
                for composite_series_key, slice_list in series_dict.items():
                    with perf_timer("first_paint.merge_batch.organize.sort_series"):
    ...
    def merge_batch(
        ...
        perf_mark(
            "first_paint.merge_batch.input_summary",
            input_datasets=len(datasets),
            new_datasets=len(datasets_new),
            skipped_files=result.skipped_file_count,
        )
        with perf_timer("first_paint.merge_batch.organize_batch"):
        ...
        perf_mark(
            ...
            appended_series=len(result.appended_series),
    ...
    def get_series_multiframe_info(self, study_uid: str, series_key: str) -> MultiFrameSeriesInfo | None:
    ...
    def _sort_slices(self, slice_list: list[tuple[Dataset, str | None]]) -> list[tuple[Dataset, str | None]]:
        """
        ...
        """
        def get_sort_key(item: tuple[Dataset, str | None]) -> tuple[float, float]:
    ...
    def get_series_list(self, study_uid: str | None = None) -> list[tuple[str, str]]:
        """
    ...
    def get_file_path(self, study_uid: str, series_uid: str, instance_number: int) -> str | None:
        """
