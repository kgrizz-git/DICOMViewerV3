    ...
    *,
    ...
    Args:
        ...
        supplement_standard_tags: If True, apply :func:`supplement_export_tags_dict`
            after the union.
        include_sequences: Passed to :meth:`~core.dicom_parser.DICOMParser.get_all_tags`
            and to :func:`supplement_export_tags_dict`. False (default) reproduces the
            root-level rows only (sequences appear as summary rows); True opts into SQ
            parent rows (rendered as a single summary cell when selected on their
            own) plus their nested ``Item N`` / leaf rows (``depth > 0``, path-keyed),
            per tag-export dialog's "Include sequences" checkbox — Phase 5 lets the
            user select individual nested leaves as their own export columns, so
            these rows must reach the picker rather than being dropped here.
