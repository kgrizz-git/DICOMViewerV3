...
try:
    from utils import debug_flags as _debug_flags
except ImportError:
    _debug_flags = None

DEBUG_MEASUREMENT_DRAG: bool = bool(getattr(_debug_flags, "DEBUG_MEASUREMENT_DRAG", False))
DEBUG_MAGNIFIER: bool = bool(getattr(_debug_flags, "DEBUG_MAGNIFIER", False))
...
class MeasurementHandle(QGraphicsEllipseItem):
    ...
    def __init__(self, measurement: 'MeasurementItem', is_start: bool, color: QColor | None = None):
        """
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        """Paint the handle; skip painting when this handle is being dragged so it is hidden but still receives events."""
        if self.parent_measurement is not None and getattr(self.parent_measurement, "_dragging_handle", None) is self:
            return
    ...
    def mousePressEvent(self, event: 'QGraphicsSceneMouseEvent') -> None:
        ...
        if self.parent_measurement is not None:
            ...
            # PySide6 KeyboardModifier is a pure Flag (not IntFlag), so `!= 0` is
            # always True — use bool() to actually test whether Shift is held.
    ...
    def mouseReleaseEvent(self, event: 'QGraphicsSceneMouseEvent') -> None:
        ...
        if self.parent_measurement is not None:
            ...
            self.update()  # Repaint so this handle is visible again
            # Sync both handle positions (measurement remains selected)
    ...
    def itemChange(self, change: QGraphicsItem.GraphicsItemChange, value) -> object:
        ...
        Returns:
            Modified value
        """
        handled, result = process_measurement_handle_item_change(self, change, value)
        if handled:
            return result
        return super().itemChange(change, value)
...
class MeasurementItem(QGraphicsItemGroup):
    ...
    def __init__(self, start_point: QPointF, end_point: QPointF,
        ...
        self._dragging_handle: MeasurementHandle | None = None
        self._last_drag_pos: QPointF | None = None

        self.on_moved_callback: Callable[[], None] | None = None
        self.on_mouse_release_callback: Callable[[], None] | None = None
        # Optional callbacks for handle-drag magnifier (Shift+drag on handle)
        self.on_handle_drag_start_callback: HandleDragStartCallback | None = None
        self.on_handle_drag_move_callback: HandleDragMoveCallback | None = None
        self.on_handle_drag_end_callback: HandleDragEndCallback | None = None
    ...
    def _should_paint_selection_outline(self) -> bool:
        """True when the yellow selected outline/markers should be drawn.

        Suppressed while a handle is actively being dragged so repositioning an
        endpoint does not flash the selection highlight (in the main viewer or
        the handle-drag magnifier, which renders the same scene). Selection and
        handles are left untouched; styling returns to normal on release.
        """
        return self.isSelected() and not getattr(self, '_handle_drag_in_progress', False)

    def paint(self, painter: QPainter, option, widget=None) -> None:
        """Paint selection indicator when item is selected (yellow dashed line)."""
        paint_selection = self._should_paint_selection_outline()
    ...
    def update_distance(self, pixel_spacing: tuple[float, float] | None = None) -> None:
    ...
    def itemChange(self, change: QGraphicsItem.GraphicsItemChange, value) -> object:
        ...
        handled, result = process_measurement_group_item_change(self, change, value)
        if handled:
            return result
        return super().itemChange(change, value)
