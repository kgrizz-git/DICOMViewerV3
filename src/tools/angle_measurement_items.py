...
def format_angle_label(degrees: float) -> str:
    ...
    if degrees >= 100.0 or degrees == 0.0:  # NOSONAR(S1244): zero-angle label-format sentinel
        return f"{degrees:.1f}°"
    return f"{degrees:.2f}°"
...
class DraggableAngleMeasurementText(QGraphicsTextItem):
    ...
    def __init__(
        self,
        measurement: AngleMeasurementItem | None,
...
class AngleVertexHandle(QGraphicsEllipseItem):
    ...
    def __init__(self, measurement: AngleMeasurementItem, vertex_index: int, color: QColor | None = None) -> None:
        handle_size = 6.0
        ...
        if self.parent_measurement is not None:
            ...
            if start_callback is not None:
                try:
                    ...
                    # PySide6 KeyboardModifier is a pure Flag (not IntFlag), so `!= 0`
                    # is always True — use bool() to actually test whether Shift is held.
...
class AngleMeasurementItem(QGraphicsItemGroup):
    ...
    def __init__(
        ...
        self._dragging_handle: AngleVertexHandle | None = None
